"""Build and verify the repository-free OpenEvo launcher distribution."""
...
LAUNCHER_DISTRIBUTION_ROOT = "openevo-launcher"
LAUNCHER_DISTRIBUTION_SUFFIX = ".tar.gz"
...
MANAGED_WRAPPER_MARKER = "# managed by OpenEvo launcher installer"
...
def _installer_readme(version: str) -> bytes:
    return f"""OpenEvo launcher {version}
...
the OpenEvo repository are not required on this computer.
""".encode("utf-8")


INSTALLER_SCRIPT = r'''#!/bin/sh
# OpenEvo repository-free per-user installer
set -eu
umask 077
...
Installs the versioned OpenEvo launcher under ~/.local by default.
EOF
}
...
WRAPPER_MARKER = "# managed by OpenEvo launcher installer"
...
def report_error(error_type, error, traceback):
    if issubclass(error_type, KeyboardInterrupt):
        ...
        return
    print(f"OpenEvo installation failed: {error}", file=sys.stderr)
...
case "$distribution_id" in
  ''|*[!0-9a-f]*) echo "OpenEvo active launcher receipt is invalid." >&2; exit 70 ;;
esac
...
if [ ! -f "$release_root/openevo.pyz" ] || [ ! -f "$release_root/openevo-server.oevobundle" ]; then
  echo "OpenEvo active launcher release is incomplete." >&2
  exit 71
fi
...
print(f"Installed OpenEvo {manifest['product_version']} ({distribution_id[:12]}).")
print(f"Command: {wrapper_path}")
if os.fspath(bin_root) not in os.environ.get("PATH", "").split(os.pathsep):
    print(f"Add {bin_root} to PATH, then run: openevo webui")
else:
    print("Run: openevo webui")
PY
'''
...
def build_launcher_distribution(
    ...
    *,
    ...
    if not target.name.endswith(LAUNCHER_DISTRIBUTION_SUFFIX):
        raise LauncherDistributionError(
            f"launcher distribution output must end with {LAUNCHER_DISTRIBUTION_SUFFIX}"
        )
    ...
    try:
        with tarfile.open(archive_path, "r:gz") as archive:
            ...
            if len(names) != len(set(names)):
                raise LauncherDistributionError("launcher archive contains duplicate entries")
            manifest_name = f"{LAUNCHER_DISTRIBUTION_ROOT}/manifest.json"
            if names.count(manifest_name) != 1:
                raise LauncherDistributionError("launcher archive has no unique manifest")
            ...
            for item in files:
                ...
                if not member.isfile() or member.size != item["byte_size"]:
                    raise LauncherDistributionError(f"launcher archive file is invalid: {name}")
                source = archive.extractfile(member)
                if source is None:
                    raise LauncherDistributionError(f"launcher archive file could not be read: {name}")
                digest = hashlib.sha256()
                read_bytes = 0
                while chunk := source.read(1024 * 1024):
