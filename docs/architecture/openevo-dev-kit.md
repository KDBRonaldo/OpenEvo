...
OpenEvo does not ship a separate Dev Kit or product CLI. Developer workflows
wrap OpenEvo Core through source checkout utilities, Python-facing Core modules,
the `openevo-backend` server-side launcher, benchmark adapters, method
development helpers, artifact inspection, and regression-test fixtures.
