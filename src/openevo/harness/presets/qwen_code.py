...
from openevo.runtime.base import BaseRuntime, RUNTIME_AGENT_LOG_DIR
...
class QwenCodeHarness(BaseHarness):
    ...
    async def setup(self, runtime: BaseRuntime) -> None:
        ...
        if self.mcp_servers:
            ...
            await runtime.exec(
                f"cat > {self._qwen_dir}/settings.json << 'OPENEVO_CFG'\n{config_json}\nOPENEVO_CFG"
            )
        ...
        for skills_path in self.effective_skill_paths():
            await runtime.exec(
                ...
                f"cp -r {shlex.quote(skills_path)}/* {self._qwen_dir}/skills/ 2>/dev/null || true"
            )
