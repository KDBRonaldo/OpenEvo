...
Each harness gets the same image at `/openevo/session/workspace/openevo_stars.png`,
...
in the rollout/gateway logs.
    ...
    uv run python examples/research-benchmarks/count_stars/run.py                 # docker (default)
    uv run python examples/research-benchmarks/count_stars/run.py --backend apptainer
"""
...
IMAGE_FILE = EXAMPLE_DIR / "assets" / "openevo_stars.png"
TOPOLOGY = EXAMPLE_DIR / "topology.yaml"
RUNTIME_IMAGE = "openevo-localhost-count-stars:latest"
RUNTIME_IMAGE_PATH = "/openevo/session/workspace/openevo_stars.png"
NUM_SAMPLES = 4
...
INSTRUCTION = """\
Use your image viewing tool to inspect `/openevo/session/workspace/openevo_stars.png`.
...
_WORKSPACE_PREPARE = (
    ...
    "git init -q && "
    "git config user.email 'openevo@test' && "
    "git config user.name 'OpenEvo'"
)
