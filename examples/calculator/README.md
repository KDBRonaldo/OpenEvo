...
```bash
uv run python -m openevo.rollout.server --config examples/calculator/topology.yaml
uv run python -m openevo.gateway.server --config examples/calculator/topology.yaml --node-id localhost-node-01
uv run python -m openevo.gateway.server --config examples/calculator/topology.yaml --node-id localhost-node-02
```
