...
def _ensure_create_operation(
    ...
    *,
    ...
    if bound:
        ...
        if (
            operation.workspace_upload_finalize is not None
            and operation.workspace_upload_finalize.state
            is CoreWorkspaceUploadFinalizeStateV1.APPLIED
        ):
            assert operation.workspace_upload_finalize.outcome is not None
            _ensure_immutable_authority_transition(
                create_immutable,
                _patch_immutable_authority(operation.workspace_upload_finalize.outcome.project),
                allow_project_patch=True,
