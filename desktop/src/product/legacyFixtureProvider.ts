...
export class FixtureDesktopProductProvider implements DesktopProductProvider {
  ...
  async updateProject(projectId: string, input: ProjectPatchV1, intent: ProductResourceMutationIntent): Promise<ProjectV1> {
    ...
    const taskOnlyUpdate = input.task !== undefined
      && Object.keys(input).every((key) => key === "task");
    const updated = projectV1Schema.parse({
      ...current,
      ...input,
      state: wasActive && !taskOnlyUpdate ? "draft" : current.state,
      remote: wasActive && !taskOnlyUpdate ? null : current.remote,
      etag: ETAG_D,
      updated_at: NOW,
    });
    this.projects = this.projects.map((project) => (project.project_id === projectId ? updated : project));
    if (wasActive && !taskOnlyUpdate) {
  ...
  async startRun(intent: ProductRunIntent): Promise<RunV1> {
    ...
    const run = runV1Schema.parse({
      ...
      task_snapshot: { id: `task-snapshot-${runNumber}`, kind: "task", content_sha256: sha256Utf8V2(canonicalJsonV2(project.task)), created_at: NOW },
    ...
    this.schedule(2, () => this.whileRunActive(runId, () => {
      ...
      this.appendLog(
        runId,
        "agent",
        `Research execution is using the selected workspace and evidence sources. Task: ${project.task.title}. ${project.task.objective}`,
      );
    }));
