...
describe("DesktopProductApp", () => {
  ...
  it("keeps the renderer-owned sample visible and real mutations closed after initial sync failure", async () => {
    ...
    expect(
      document.querySelector('[data-testid="sample-research-workspace"]'),
    ).not.toBeNull();
    const fallbackSwitcher =
      document.querySelector<HTMLSelectElement>("#project-switcher");
    expect(fallbackSwitcher?.disabled).toBe(false);
    expect(
      Array.from(fallbackSwitcher?.options ?? []).filter((option) =>
    ...
    expect(screenText()).toContain(
      "The linearized fit systematically misses high-concentration observations",
    );
    await clickButton("Evolution");
    expect(
      document.querySelector('[data-testid="sample-evolution-workspace"]'),
    ).not.toBeNull();
    ...
    const proteinOption = Array.from(fallbackSwitcher?.options ?? []).find(
      (option) =>
        option.textContent?.includes("Protein Stability Evidence Review"),
    );
    if (!fallbackSwitcher || !proteinOption)
      throw new Error("Second fallback sample was not found.");
    await act(async () => {
  ...
  it("keeps sample and real project identities isolated while switching between them", async () => {
    ...
    const sampleOption = Array.from(switcher.options).find((option) =>
      option.textContent?.includes("[Demo] Enzyme Kinetics Model Review"),
    );
    ...
    if (!sampleOption || !realOption) {
      throw new Error(
        "Sample and real project options must both be discoverable.",
      );
    }
    ...
    expect(
      Array.from(switcher.options).some(
        (option) => option.value === realOption.value,
      ),
    ).toBe(true);
  ...
  it("switches between two renderer-owned demos without provider mutation", async () => {
    ...
    const switcher =
      document.querySelector<HTMLSelectElement>("#project-switcher");
    if (!switcher) throw new Error("Project switcher was not found.");
    const enzymeOption = Array.from(switcher.options).find((option) =>
      option.textContent?.includes("[Demo] Enzyme Kinetics Model Review"),
    );
    const proteinOption = Array.from(switcher.options).find((option) =>
      option.textContent?.includes("[Demo] Protein Stability Evidence Review"),
    );
    ...
    await clickButton("Task 1");
    expect(screenText()).toContain(
      "Confounding and scale differences invalidate the original ranking",
    );
    await clickButton("Evolution");
    await clickButton("Changes");
    expect(screenText()).toContain("ER-PS-2 → ER-PS-3");
  ...
  it("reports readiness once after the initial product snapshot commits", async () => {
    ...
    expect(onReady).toHaveBeenCalledTimes(1);
    expect(screenText()).toContain("What should the agent do next?");
  });
  ...
  it("shows bounded session output and filters agent and evolution records", async () => {
    ...
    expect(
      document.querySelectorAll(".run-timeline li").length,
    ).toBeGreaterThan(0);
    const renderedTimeline = [
      ...document.querySelectorAll<HTMLElement>(".run-timeline li"),
    ];
    const renderedSequences = renderedTimeline.map((entry) =>
      Number(entry.dataset.sequence),
    );
    expect(renderedSequences).toEqual(
      [...renderedSequences].sort((left, right) => left - right),
    );
    expect(
      renderedTimeline.every(
        (entry) =>
    ...
    const renderedLogs = [
      ...document.querySelectorAll<HTMLElement>(".session-output-entry"),
    ];
  ...
  it("keeps the Daemon project head authoritative while the project projection refreshes", async () => {
    ...
    if (current.status !== "fresh")
      throw new Error("Expected a fresh fixture snapshot.");
    const project = current.snapshot.projects[0];
    const predecessor = current.snapshot.runs[0]?.pinned_revision;
    if (!project?.remote || !predecessor)
      throw new Error("Expected revision fixtures.");
    ...
    expect(
      document.querySelector(".revision-node.active")?.textContent,
    ).toContain("Project Head 1");
    expect(screenText()).not.toContain("Project Head 2");
    expect(document.querySelectorAll(".revision-node")).toHaveLength(1);
    expect(
      document.querySelector(".artifact-list-heading")?.textContent,
  ...
  it("refreshes live session output after authoritative stream updates", async () => {
    ...
    expect(screenText()).toContain(
      "Session admitted with an immutable project snapshot.",
    );
    expect(screenText()).not.toContain(
      "Research execution is using the selected workspace",
    );
  ...
  it("saves the entered task and starts the session with one action", async () => {
    vi.useFakeTimers();
    provider = createFixtureDesktopProductProvider({
      startOnline: true,
      stepDelayMs: 20,
    ...
    setInput("Task title", "Fix the authentication regression");
    setInput(
      "Task instructions",
      "Diagnose the failing login tests, implement the fix, and run the focused suite.",
    );
    ...
    expect(updateProject).toHaveBeenCalledWith(
      ...
      {
        task: {
          title: "Fix the authentication regression",
          objective:
            "Diagnose the failing login tests, implement the fix, and run the focused suite.",
        },
      },
    ...
    await advance(45);
    expect(screenText()).toContain("Task: Fix the authentication regression.");
    const session =
      [
        ...document.querySelectorAll<HTMLButtonElement>(".session-open-button"),
      ].at(0) ?? null;
    expect(session).not.toBeNull();
    await act(async () => session!.click());
    expect(screenText()).toContain("Fix the authentication regression");
    expect(screenText()).toContain(
      "Diagnose the failing login tests, implement the fix, and run the focused suite.",
    );
    expect(screenText()).toContain("Session conversation");
    expect(screenText()).toContain("You");
  });
  ...
  it("keeps polling serial and rejects its late result after switching projects", async () => {
    ...
    const stale = {
      ...
      snapshot: {
        ...current.snapshot,
        projects: current.snapshot.projects.filter(
          (item) => item.project_id === "project-fixture-1",
        ),
      },
    };
    const pendingPoll =
      deferred<Awaited<ReturnType<FixtureDesktopProductProvider["refresh"]>>>();
    ...
    await flush();
    expect(labelledControl<HTMLInputElement>("Task title", "input").value).toBe(
  ...
  it("never publishes a superseded run-log request into the next run", async () => {
    ...
    const staleLogs = (await provider.getRunLogs(previousRun.id)).map(
      (entry, index) => ({
        ...entry,
  ...
  it("hides resolved output synchronously when a colliding attempt identity becomes current", async () => {
    ...
    const oldSnapshot = withRunOutputIdentity(
      initial.snapshot,
      "run-collision",
      null,
    );
    const nextSnapshot = withRunOutputIdentity(
      initial.snapshot,
      "run-collision",
      "no-attempt",
    );
    const oldLogs = relabelLogs(
      baseLogs,
      "run-collision",
      null,
      "OLD NO-ATTEMPT OUTPUT",
    );
    ...
    expect(
      document.querySelector('[aria-label="Refreshing session output"]'),
    ).not.toBeNull();
  ...
  it("never publishes an old colliding request after a rapid attempt switch", async () => {
    ...
    await act(async () => {
      oldRequest.resolve(
        relabelLogs(baseLogs, "run-collision", null, "STALE NO-ATTEMPT OUTPUT"),
      );
      await Promise.resolve();
      await Promise.resolve();
    });
  ...
  it("gates sessions offline and completes first-time workspace setup", async () => {
    ...
    setInput("Initial task title", "Compare catalyst candidates");
    setInput(
      "Initial task instructions",
      "Rank candidates using reproducible evidence.",
    );
    expect(
      labelledControl<HTMLInputElement>("Codex model", "input").value,
    ).toBe("gpt-5.5");
    expect(
      labelledControl<HTMLSelectElement>("Reasoning effort", "select").value,
    ).toBe("high");
    expect(screenText()).not.toContain("Hugging Face model");
    await clickButton("Prepare evolution");
    await clickButton("Save and activate");
    expect(labelledControl<HTMLInputElement>("Task title", "input").value).toBe(
      "Compare catalyst candidates",
    );
  ...
  it("names the first missing required field in workspace and project setup", async () => {
    ...
    expect(button("Save workspace").getAttribute("aria-describedby")).toBe(
    ...
    const objective = labelledControl<HTMLTextAreaElement>(
      "Initial task instructions",
      "textarea",
    );
    expect(objective.required).toBe(true);
    expect(screenText()).toContain("Describe the research objective.");
    expect(button("Prepare evolution").getAttribute("aria-describedby")).toBe(
  ...
  it("shows the authoritative retired state after editing an active project", async () => {
    ...
    setInput("Project name", "Project requiring fresh activation");
  ...
  it("keeps current-project settings separate from the next task draft", async () => {
    provider = createFixtureDesktopProductProvider({ startOnline: true });
    const updateProject = vi.spyOn(provider, "updateProject");
    root = await renderProduct(provider);

    await clickAria("Project settings");
    expect(screenText()).toContain("Current project settings");
    expect(screenText()).not.toContain("Initial task title");
    expect(screenText()).not.toContain("Initial task instructions");
    setInput("Project name", "Current project only");
    await clickButton("Save changes");

    expect(updateProject).toHaveBeenCalled();
    expect(updateProject.mock.calls.at(-1)?.[1]).not.toHaveProperty("task");
  });
  ...
  it("preserves an older project's unpinned Codex reasoning effort on unrelated edits", async () => {
    ...
    setInput("Project name", "Project retaining the Codex default");
    const firstEvolutionToggle = document.querySelector<HTMLInputElement>(
      ".target-toggle input[role='switch']",
    );
    if (!firstEvolutionToggle)
      throw new Error("Evolution target switch was not found.");
  ...
  it.each([
    ...
    "saves the selected Codex reasoning effort: %s",
    async (_label, initialEffort, selection) => {
      provider = createFixtureDesktopProductProvider({
        startOnline: true,
        projectExecutionMode: "codex_subscription_transcript",
      ...
      setInput("Project name", `Codex effort ${selection}`);
      await clickButton("Save");
  ...
  it("resets Codex reasoning effort to the saved value and preserves it across mode switching", async () => {
    ...
    setInput("Project name", "Project retaining selected Codex effort");
    await clickButton("Save");
  ...
  it("keeps new-project setup open and activates only the evolution targets selected by the user", async () => {
    ...
    if (before.status !== "fresh")
      throw new Error("Expected a fresh fixture snapshot.");
    expect(before.snapshot.capability).toMatchObject({
      status: "ready",
      executionMode: "self-deployed",
    });
    root = await renderProduct(provider);
    ...
    setInput(
      "Initial task instructions",
      "Keep subscription defaults scoped to this new project.",
    );
    await clickButton("Prepare evolution");
    ...
    expect(
      [
        ...document.querySelectorAll<HTMLOptionElement>(
          "#codex-model-suggestions option",
        ),
      ].map((option) => option.value),
    ).toEqual(["gpt-5.5", "gpt-5.3-codex-spark"]);
    ...
    const firstToggle = document.querySelector<HTMLInputElement>(
      '.target-toggle[data-target-id="text_memory"] input[role="switch"]',
    );
    ...
    if (
      !(firstTrack instanceof HTMLElement) ||
      !firstTrack.classList.contains("switch-track")
    ) {
      throw new Error(
        "Evolution target switch track was not adjacent to its checkbox.",
      );
    }
    ...
    expect(
      document.querySelector<HTMLSelectElement>(
        'select[aria-label="Text memory method"]',
      )?.value,
    ...
    if (prepared.status !== "fresh")
      throw new Error("Expected a fresh fixture snapshot.");
    const draft = prepared.snapshot.projects.find(
      (project) =>
    ...
    if (activated.status !== "fresh")
      throw new Error("Expected a fresh fixture snapshot.");
    const created = activated.snapshot.projects.find(
      (project) =>
        project.task.objective ===
        "Keep subscription defaults scoped to this new project.",
    );
  ...
  it("shows only release-supported modes when creating a project", async () => {
    ...
    expect(dialog?.textContent).not.toContain(
      "not available in this OpenEvo Desktop release",
    );
    expect(screenText()).not.toContain("Hugging Face model");
  });
  ...
  it("keeps a saved unavailable mode visible, blocks mutations, and lets the user switch away", async () => {
    ...
    expect(button("Start session").disabled).toBe(true);
    expect(button("Start session").title).toContain(
      "not available in this OpenEvo Desktop release",
    );
    await clickAria("Project settings");
    ...
    if (refreshed.status !== "fresh")
      throw new Error("Expected a fresh fixture snapshot.");
    expect(refreshed.snapshot.projects[0]?.execution.mode).toBe(
  ...
  it("blocks activation for a saved release-unavailable mode", async () => {
    ...
    expect(button("Activate project").title).toContain(
      "not available in this OpenEvo Desktop release",
    );
  ...
  it("does not reuse another project's same-mode capabilities after a new-project mode switch", async () => {
    ...
    expect(button("Self-deployed").getAttribute("aria-checked")).toBe("true");
    expect(
      labelledControl<HTMLInputElement>("Hugging Face model", "input").value,
    ).toBe("Qwen/Qwen3-8B");
    expect(document.querySelectorAll(".target-toggle")).toHaveLength(0);
    setInput(
      "Initial task instructions",
      "Keep self-deployed defaults scoped to this new project.",
    );
    await clickButton("Prepare evolution");
    ...
    if (refreshed.status !== "fresh")
      throw new Error("Expected a fresh fixture snapshot.");
    const created = refreshed.snapshot.projects.find(
      (project) =>
        project.task.objective ===
        "Keep self-deployed defaults scoped to this new project.",
    );
  ...
  it("resets a mounted project drawer before creating and never exposes unavailable snapshot sync", async () => {
    ...
    expect(
      labelledControl<HTMLInputElement>("Project name", "input").value,
    ).toBe("New research project");
    expect(button("Subscription").getAttribute("aria-checked")).toBe("true");
    expect(
      labelledControl<HTMLInputElement>("Codex model", "input").value,
    ).toBe("gpt-5.5");
    ...
    setInput("Initial task instructions", "Create without project A state.");
    await clickButton("Prepare evolution");
    const refreshed = await provider.refresh();
    if (refreshed.status !== "fresh")
      throw new Error("Expected a fresh fixture snapshot.");
    const created = refreshed.snapshot.projects.find(
      (project) => project.task.objective === "Create without project A state.",
    );
  ...
  it("commits the later revision and pins it in the next session", async () => {
    ...
    expect(screenText()).toContain("Latest session complete");
    expect(screenText()).toContain(
      "Project Head 3 contains this session's selected evolution outputs.",
    );
    expect(
      Array.from(
    ...
    ).toContain("Evolution output");
    expect(screenText()).not.toContain("Successor");

    const completed = await provider.refresh();
    if (completed.status !== "fresh")
      throw new Error("Fixture refresh was not fresh.");
    const evolvedRun = completed.snapshot.runs.find(
      (run) => run.status === "succeeded" && run.id !== "run-fixture-1",
    );
    ...
    const evolvedArtifacts = completed.snapshot.artifacts.filter(
      (artifact) => artifact.run_id === evolvedRun?.id,
    );
    expect(evolvedArtifacts).toHaveLength(3);
    expect(
      evolvedArtifacts.every(
        (artifact) => artifact.produced_revision.generation === 3,
      ),
    ).toBe(true);

    await clickButton("Session 1");
    expect(screenText()).toContain("Session conversation");
    expect(screenText()).toContain("Session 1");
    await clickButton("Back to project");

    await clickButton("Session 2");
    expect(screenText()).toContain("Session conversation");
    expect(screenText()).toContain("Session 2");
    await clickButton("Back to project");
  ...
  it("keeps Desktop project identity distinct from Core run and artifact identity", async () => {
    ...
    if (!project?.remote)
      throw new Error("Fixture project did not have a remote identity.");

    expect(project.project_id).not.toBe(project.remote.core_project_id);
    expect(
      refreshed.snapshot.runs.every(
        (run) => run.project_id === project.remote?.core_project_id,
      ),
    ).toBe(true);
    expect(
      refreshed.snapshot.artifacts.every(
        (artifact) => artifact.project_id === project.remote?.core_project_id,
      ),
    ).toBe(true);
  ...
  it("keeps a terminal fixture run immutable when scheduled steps fire later", async () => {
    ...
    const run = await provider.startRun({
      ...
      actionId: "state-machine-start",
    });
    await provider.cancelRun(run.id, {
      etag: run.etag,
      streamEpoch: initial.snapshot.stream.epoch,
      actionId: "state-machine-cancel",
    });

    vi.advanceTimersByTime(25);
    const transitioned = await provider.refresh();
    if (transitioned.status !== "fresh")
      throw new Error("Fixture refresh was not fresh.");
    const current = transitioned.snapshot.runs.find(
  ...
  it("shows artifact content, changes, document tabs, and truncation", async () => {
    ...
    expect(contentRoot?.dataset.artifactContentSha256).toMatch(
      /^[a-f0-9]{64}$/,
    );
    ...
    const documentTabs = [
      ...document.querySelectorAll<HTMLElement>(".document-tabs [role=tab]"),
    ];
    ...
    const documentPanel =
      document.querySelector<HTMLElement>(".artifact-document");
    ...
    expect(diff?.dataset.previousArtifactContentSha256).toMatch(
      /^[a-f0-9]{64}$/,
    );
  ...
  it("refuses content and changes cross-wired to another selected artifact", async () => {
    ...
    await flush();
    expect(screenText()).toContain(
      "Artifact content identity does not match the selected artifact.",
    );
    expect(document.querySelector(".artifact-document")).toBeNull();
    await clickButton("Changes");
    await flush();
    expect(screenText()).toContain(
      "Artifact change identity does not match the selected artifact.",
    );
  ...
  it("refuses a diff whose previous artifact identity is unrelated to the selection", async () => {
    ...
    await flush();
    expect(screenText()).toContain(
      "Artifact change history does not match the selected artifact.",
    );
    expect(document.querySelector(".diff-hunk")).toBeNull();
  });
  ...
  it("resumes an incomplete evolution setup after refresh and blocks an unsupported saved method", async () => {
    ...
    const staleToggle = document.querySelector<HTMLInputElement>(
      '.target-toggle[data-target-id="text_memory"] input[role="switch"]',
    );
    if (!staleToggle)
      throw new Error("Unsupported target toggle was not found.");
    await act(async () => staleToggle.click());
    expect(staleToggle.checked).toBe(false);
    await act(async () => staleToggle.click());
    const repairedMethod = document.querySelector<HTMLSelectElement>(
      '.target-toggle select[aria-label="Text memory method"]',
    );
  ...
  it("preserves accepted existing methods and offers supported Core selection resolvers", async () => {
    ...
    const hiddenOption = Array.from(
      document.querySelectorAll<HTMLOptionElement>("option"),
    ).find((item) => item.value === "hidden_text_memory");
  ...
  it("preserves explicit evolution config across mode changes without reusing old-mode capabilities", async () => {
    ...
    if (!explicitTargets)
      throw new Error(
        "Expected an existing project with explicit evolution targets.",
      );
    root = await renderProduct(provider);
    ...
    if (refreshed.status !== "fresh")
      throw new Error("Expected a fresh fixture snapshot.");
    expect(refreshed.snapshot.projects[0]?.evolution.targets).toEqual(
      explicitTargets,
    );
  });
  ...
  it("retries unavailable capabilities for the same project and mode without losing the draft", async () => {
    ...
    await clickAria("Project settings");
    setInput("Project name", "Capability retry draft");
    expect(screenText()).toContain(
      "Capabilities are unavailable for this project and mode.",
    );
  ...
  it("edits every ordinary field in a closed remote method config schema", async () => {
    ...
    if (refreshed.status !== "fresh")
      throw new Error("Fixture refresh was not fresh.");
    expect(
      refreshed.snapshot.projects[0]?.evolution.targets.text_memory?.config,
    ).toEqual({
  ...
  it("deep-merges method defaults for display while saving only the partial user override", async () => {
    ...
    expect(
      labelledControl<HTMLInputElement>("Reflection prompt", "input").value,
    ).toBe("Keep durable findings.");
    expect(labelledControl<HTMLInputElement>("Iterations", "input").value).toBe(
      "5",
    );
    expect(
      labelledControl<HTMLInputElement>("Minimum score", "input").value,
    ).toBe("0.5");
    setInput("Minimum score", "0.8");
    await clickButton("Save");
  ...
  it("renders typed queued, succeeded, failed, and cancelled run outcomes with recovery", async () => {
    ...
    const retryRun = vi.fn(
      (_runId: string, _intent: ProductResourceMutationIntent) =>
    ...
    expect(screenText()).toContain(
      "The model worker could not load the selected model.",
    );
    ...
    expect(
      document.querySelectorAll('[role="cell"]').length,
    ...
    await act(async () => {
      retryRequest.reject(
        new DesktopProductUserError("The failed session could not be retried."),
      );
      await Promise.resolve();
      await Promise.resolve();
    });
    ...
    expect(retryRun.mock.calls[1]?.[1].actionId).not.toBe(
      retryRun.mock.calls[0]?.[1].actionId,
    );
    expect(startRun).not.toHaveBeenCalled();
  });

  it.each(["accepted retry persistence", "rejected retry clear"])(
    "preserves restart guidance after uncertain %s",
    async () => {
      provider = createFixtureDesktopProductProvider({
        startOnline: true,
        seedCompletedRun: true,
      ...
      expect(screenText()).not.toContain(
        "The retry outcome is not yet confirmed.",
      );
  ...
  it("shows the failed preview outcome and retries by appending an attempt to the same run", async () => {
    ...
    expect(screenText()).not.toContain(
      "Remote capabilities are unavailable for this project and mode.",
    );
    expect(screenText()).toContain(
    ...
    expect(
      after.snapshot.runs[0]?.attempts.map((attempt) => attempt.status),
    ).toEqual(["failed", "queued"]);
    expect(screenText()).toContain(
      "The retry was admitted on the same session.",
    );
  });
  ...
  it("clears an unknown retry error when refresh proves the same run advanced", async () => {
    ...
    const retryRun = vi.fn(async () => {
      throw new DesktopProductAmbiguousMutationError(
        "The retry response was lost.",
      );
    });
    Object.assign(provider, { retryRun });
    root = await renderProduct(provider);
  ...
  it("keeps an accepted retry response visible when the following snapshot omits the run", async () => {
    ...
    const advancedRun = advancedSnapshot.runs.find(
      (run) => run.id === "run-failed-model",
    );
  ...
  it("does not reconcile an accepted retry from a different appended attempt or project", async () => {
    ...
    const accepted = acceptedSnapshot.runs.find(
      (run) => run.id === "run-failed-model",
    );
    if (!accepted?.current_attempt)
      throw new Error("Expected an accepted retry.");
    ...
    refresh.mockResolvedValueOnce({
      ...
      snapshot: {
        ...acceptedSnapshot,
        runs: acceptedSnapshot.runs.map((run) =>
          run.id === accepted.id
            ? {
                ...run,
  ...
  it("does not treat a temporarily absent run as retry advancement", async () => {
    ...
    await clickButton("Retry session");
    await flush();
    const retryCalls = retryRun.mock.calls as unknown as Array<
      [string, ProductResourceMutationIntent]
    >;
    ...
    vi.spyOn(provider, "refresh").mockResolvedValueOnce({
      ...
      snapshot: {
        ...failed.snapshot,
        runs: failed.snapshot.runs.filter(
          (run) => run.id !== "run-failed-model",
        ),
      },
    });
  ...
  it("does not treat status or ETag churn without a new attempt as retry advancement", async () => {
    ...
    vi.spyOn(provider, "refresh").mockResolvedValueOnce({
      ...
      snapshot: {
        ...failed.snapshot,
        runs: failed.snapshot.runs.map((run) =>
          run.id === "run-failed-model"
  ...
  it("starts retry reconciliation polling only after the request becomes ambiguous", async () => {
    ...
    await act(async () => {
      retryRequest.reject(
        new DesktopProductAmbiguousMutationError(
          "The retry response was lost.",
        ),
      );
      await Promise.resolve();
    ...
    await advance(1_005);
    expect(refresh.mock.calls.length).toBeGreaterThan(
      refreshesAfterAmbiguousResult,
    );
  });
      ...
      retryRequest.reject(
        new DesktopApiError(
          apiErrorV1Schema.parse({
            ...
            message:
              "The exact retry was rejected after reconciliation started.",
            severity: "warning",
            category: "run",
            retryable: false,
  ...
  it("restores an exact ambiguous retry after the product renderer remounts", async () => {
    ...
    const failed = await provider.refresh();
    if (failed.status !== "fresh")
      throw new Error("Expected a fresh failed fixture.");
    const failedRun = failed.snapshot.runs.find(
      (run) => run.id === "run-failed-model",
    );
    ...
    const retryRun = vi.fn(async () => {
      throw new DesktopProductAmbiguousMutationError(
        "Still reconciling the original retry.",
      );
    });
  ...
  it("does not let retry completion clear a newer operation error", async () => {
    ...
    if (beforeConnect.status !== "fresh")
      throw new Error("Expected a fresh fixture snapshot.");
    const profile = beforeConnect.snapshot.profiles[0];
    const failedRun = beforeConnect.snapshot.runs.find(
      (run) => run.id === "run-failed-model",
    );
    if (!profile || !failedRun)
      throw new Error("Expected profile and failed run fixtures.");
    await act(async () => {
      await provider?.connectProfile(profile.profile_id, {
    ...
    vi.spyOn(provider, "cancelOperation").mockRejectedValueOnce(
      new DesktopProductUserError(
        "The connection operation could not be cancelled.",
      ),
    );
  ...
  it("shows every selected revision member, including multiple artifacts for one target, in stable order", async () => {
    ...
    const artifactNames = Array.from(
      document.querySelectorAll(".artifact-list-item strong"),
    ...
    const artifactSummaries = Array.from(
      document.querySelectorAll(".artifact-list-item small"),
      (item) => item.textContent,
    );
  ...
  it("does not infer session evolution outputs from an incomplete artifact collection", async () => {
    ...
    expect(screenText()).toContain("Artifact collection is incomplete");
    expect(screenText()).toContain(
      "Evolution output status is unavailable until all artifact pages are loaded.",
    );
    ...
    expect(screenText()).not.toContain(
      "contains this session's evolution outputs",
    );
    expect(button("Refetch artifacts").disabled).toBe(false);
  ...
  it("reloads typed 409/410/412 failures without replaying stale mutations", async () => {
    ...
    await clickAria("Project settings");
    setInput("Project name", "Draft retained across an etag refresh");
    provider.failNextProjectSaveWithStatus(412);
    await clickButton("Save");
    expect(
      labelledControl<HTMLInputElement>("Project name", "input").value,
    ).toBe("Draft retained across an etag refresh");
  ...
  it("keeps update action identities across uncertain responses and replaces them after a changed precondition", async () => {
    ...
    await clickAria("Project settings");
    setInput("Project name", "Keep one project update identity");
    ...
    await clickAria("Project settings");
    setInput("Project name", "Use a new identity after editing");
    provider.failNextProjectSaveWithStatus(412);
    await clickButton("Save");
    await clickButton("Save");
    ...
    await clickButton("Save workspace");
    await clickButton("Save workspace");
    expect(provider.profileUpdateActionIds()[0]).toBe(
      provider.profileUpdateActionIds()[1],
    );
  });
  ...
  it("keeps create action identities when profile and project responses are uncertain", async () => {
    ...
    await flush();
    await clickButton("Save workspace");
    expect(provider.profileCreateActionIds()[0]).toBe(
      provider.profileCreateActionIds()[1],
    );
    ...
    await clickAria("Create project");
    setInput("Initial task instructions", "Keep one project create identity.");
    await clickButton("Folder snapshot");
    ...
    await clickButton("Prepare evolution");
    expect(provider.projectCreateActionIds()[0]).toBe(
      provider.projectCreateActionIds()[1],
    );
  });

  it.each([409, 412] as const)(
    "retries activation after HTTP %s without creating another project",
    async (status) => {
      provider = createFixtureDesktopProductProvider({ startOnline: true });
      root = await renderProduct(provider);
      ...
      setInput(
        "Initial task instructions",
        `Activate the authoritative project after ${status}.`,
      );
      ...
      expect(provider.projectActivationActionIds()[0]).not.toBe(
        provider.projectActivationActionIds()[1],
      );
      expect(document.querySelector('[role="dialog"]')).not.toBeNull();
      expect(screenText()).toContain("Remote evolution methods are ready");
    },
  ...
  it("uses a single-column bounded System layout at the 760px minimum window", async () => {
    ...
    expect(
      Array.from(grid?.children ?? []).every((child) =>
        child.classList.contains("product-panel"),
      ),
    ).toBe(true);
  });
  ...
  it("replays an uncertain profile create by idempotency key before adopting it", async () => {
    ...
    expect(provider.profileCreateActionIds()[1]).toBe(
      provider.profileCreateActionIds()[0],
    );
    expect(provider.profileUpdateActionIds()).toHaveLength(0);
    expect(document.querySelector('[role="dialog"]')).toBeNull();
  ...
  it("supports radio-group selection and roving artifact tabs", async () => {
    ...
    const sourceChoices = document.querySelector<HTMLElement>(
      '[role="radiogroup"][aria-label="Research source"]',
    );
    const scratch = sourceChoices?.querySelector<HTMLButtonElement>(
      '[role="radio"][aria-checked="true"]',
    );
    if (!scratch) throw new Error("Selected research source was not found.");
    scratch.focus();
    await pressKey(scratch, "ArrowDown");
    const folder = sourceChoices?.querySelector<HTMLButtonElement>(
      '[role="radio"][aria-checked="true"]',
    );
    ...
    const modelChoices = document.querySelector<HTMLElement>(
      '[role="radiogroup"][aria-label="Model mode"]',
    );
    const selectedModel = modelChoices?.querySelector<HTMLButtonElement>(
      '[role="radio"][aria-checked="true"]',
    );
    ...
    if (!(nextModel instanceof HTMLElement))
      throw new Error("Next model tab was not focused.");
    expect(
      modelChoices?.querySelector('[role="radio"][aria-checked="true"]')
        ?.textContent,
    ).not.toBe(selectedModel.textContent);

    await clickAria("Close settings");
    await clickButton("Discard changes");
    await clickButton("Evolution");
    const artifactTabs = document.querySelector<HTMLElement>(
      '[role="tablist"][aria-label="Artifact view"]',
    );
    const content = artifactTabs?.querySelector<HTMLButtonElement>(
    ...
    expect(
      artifactTabs?.querySelector('[role="tab"][aria-selected="true"]')
        ?.textContent,
    ).toContain("Content");
    if (!(changes instanceof HTMLElement))
      throw new Error("Changes tab was not focused.");
    ...
    expect(
      document
        .querySelector('#artifact-view-panel[role="tabpanel"]')
  ...
  it("restores keyboard focus after a native folder selection settles", async () => {
    ...
    vi.spyOn(provider, "selectProjectSource").mockImplementation(
      () => pending.promise,
    );
    root = await renderProduct(provider);
    await clickAria("Project settings");
    ...
    const folder = Array.from(
      sourceChoices?.querySelectorAll<HTMLButtonElement>('[role="radio"]') ??
        [],
    ).find((choice) => choice.textContent?.includes("Folder snapshot"));
    if (!scratch || !folder)
      throw new Error("Research source controls were not found.");
    scratch.focus();
    await pressKey(scratch, "ArrowDown");
  ...
  it("exposes typed diagnostics and confirmed recovery actions for degraded services", async () => {
    ...
    expect(
      document.querySelector('button[aria-label^="Restart "]'),
    ).not.toBeNull();
  ...
  it("keeps the release System view read-only when maintenance authority is unavailable", async () => {
    ...
    expect(screenText()).toContain("Remote services need attention");
    expect(screenText()).toContain(
      "Automated maintenance is unavailable in this Preview.",
    );
  ...
  it("polls a local maintenance operation from queued through running to succeeded", async () => {
    ...
    let completed: Awaited<ReturnType<typeof provider.doctorProject>> | null =
      null;
    let lookupCount = 0;
    vi.spyOn(provider, "doctorProject").mockImplementation(
      async (...arguments_) => {
    ...
    const lookup = vi
      .spyOn(provider, "getLocalOperation")
      .mockImplementation(async () => {
        if (!completed)
          throw new Error("The completed maintenance result is unavailable.");
        lookupCount += 1;
    ...
    expect(screenText()).toContain("Running");
    expect(
      document.querySelector(".system-activity-state-icon.running"),
    ).not.toBeNull();
    ...
    expect(screenText()).toContain("Complete");
    expect(
      document.querySelector(".system-activity-state-icon.succeeded"),
    ).not.toBeNull();
  });
  ...
  it.each(["failed", "cancelled"] as const)(
    ...
    async (outcome) => {
      ...
      expect(
        activity?.querySelector(".system-activity-state-icon.succeeded"),
      ).toBeNull();
      expect(
        activity?.querySelector(`.system-activity-state-icon.${outcome}`),
      ).not.toBeNull();
      if (outcome === "failed") {
        expect(activity?.textContent).toContain(
          "The remote maintenance operation failed.",
        );
        expect(activity?.textContent).toContain("Next action:");
        expect(activity?.textContent).toContain(
          "Review System diagnostics before retrying.",
        );
        expect(button("Repair").disabled).toBe(true);
        expect(button("Repair").title).toContain(
          "required user or reconnection action",
        );
      }
    },
  );
  ...
  it("renders cancelling before a cancelled maintenance outcome", async () => {
    ...
    expect(
      document.querySelector(".system-activity-state-icon.cancelling"),
    ).not.toBeNull();
    ...
    expect(activity?.textContent).not.toContain("completed");
    expect(
      activity?.querySelector(".system-activity-state-icon.cancelled"),
    ).not.toBeNull();
  });
  ...
  it("does not render a timed-out maintenance poll as completed", async () => {
    ...
    const lookup = vi
      ...
      .mockImplementation(async () => {
        if (!running)
          throw new Error("The running maintenance result is unavailable.");
        return running;
      });
    root = await renderProduct(provider);
    ...
    expect(activity?.textContent).not.toContain("completed");
    expect(
      activity?.querySelector(".system-activity-state-icon.running"),
    ).not.toBeNull();
    expect(
      activity?.querySelector(".system-activity-state-icon.succeeded"),
    ).toBeNull();
  ...
  it("traps confirmation focus, closes on Escape, and restores both dangerous-action triggers", async () => {
    ...
    const cleanupCancel = Array.from(
      dialog?.querySelectorAll<HTMLButtonElement>("button") ?? [],
    ).find((item) => item.textContent?.trim() === "Cancel");
    if (!cleanupCancel)
      throw new Error("Cache cleanup cancel action was not found.");
  ...
  it("gates Start, selection, settings, and lifecycle mutations while System maintenance is busy", async () => {
    ...
    expect(
      document.querySelector<HTMLButtonElement>(
        'button[aria-label="Remote workspace settings"]',
      )?.disabled,
    ).toBe(true);
    expect(
      document.querySelector<HTMLButtonElement>(
  ...
  it.each(["project identity", "revision generation"] as const)(
    ...
    async (authorityChange) => {
      ...
      const nextCoreProjectId =
        authorityChange === "project identity"
      ...
      const changedProject = {
        ...
        remote: {
          ...
          active_revision: {
            ...
            generation:
              authorityChange === "revision generation"
      ...
      vi.spyOn(provider, "refresh").mockResolvedValueOnce({
        ...
        snapshot: {
          ...
          projects: current.snapshot.projects.map((project) =>
            project.project_id === changedProject.project_id
              ? changedProject
              : project,
          ),
        },
    ...
    async (operationKind) => {
      ...
      const activeOperation: LocalOperationV1 = {
        ...
        progress: {
          current: 1,
          total: 2,
          label: "Remote maintenance is running",
        },
        checks: [],
        result: null,
        error: null,
  ...
  it("fails closed when a ready project has no authoritative service status", async () => {
    ...
    expect(button("Start session").disabled).toBe(true);
    expect(button("Start session").title).toContain(
      "Remote service status is unavailable",
    );
    await clickButton("Open System");
    expect(screenText()).toContain(
  ...
  it("does not expose project A services or restart actions while project B is selected", async () => {
    ...
    expect(
      document.querySelector('button[aria-label="Restart Model service"]'),
    ).toBeNull();
  });

  it("shows only the selected project's sessions in the Research sidebar", async () => {
    provider = createFixtureDesktopProductProvider({
      startOnline: true,
      seedCompletedRun: true,
    });
    ...
    expect(document.querySelector('[aria-label="Protein Design sessions"]')).not.toBeNull();
    expect(document.querySelectorAll(".sidebar-session-item")).toHaveLength(1);
    ...
    expect(document.querySelector('[aria-label="Protein Design sessions"]')).toBeNull();
    expect(document.querySelectorAll(".sidebar-session-item")).toHaveLength(0);
  });

  it("keeps an earlier session title after starting a differently titled session", async () => {
    vi.useFakeTimers();
    provider = createFixtureDesktopProductProvider({
      startOnline: true,
      stepDelayMs: 20,
    ...
    setInput("Task title", "First named task");
    setInput("Task instructions", "Run the first named task.");
    await clickButton("Start session");
    await advance(150);

    setInput("Task title", "Second task title");
    setInput("Task instructions", "Run a different second task.");
    await clickButton("Start session");
    await advance(150);

    const titles = Array.from(
      document.querySelectorAll(".sidebar-session-item span"),
      (item) => item.textContent,
    );
    expect(titles).toContain("First named task");
    expect(titles).toContain("Second task title");
  });
  ...
  it("loads project B when selection changes while project A's drawer remains open", async () => {
    ...
    if (before.status !== "fresh")
      throw new Error("Expected a fresh fixture snapshot.");
    const projectA = before.snapshot.projects.find(
      (project) => project.project_id === "project-fixture-1",
    );
    ...
    const staleRefresh =
      deferred<Awaited<ReturnType<FixtureDesktopProductProvider["refresh"]>>>();
    vi.spyOn(provider, "refresh").mockImplementationOnce(
    ...
    expect(
      labelledControl<HTMLInputElement>("Project name", "input").value,
    ).toBe("Second research project");
    expect(screenText()).toContain(
      "Edit the next task from the Research workspace.",
    );
    expect(button("Subscription").getAttribute("aria-checked")).toBe("true");
    expect(
      labelledControl<HTMLInputElement>("Codex model", "input").value,
    ...
    setInput("Project name", "Updated project B");
    ...
    if (refreshed.status !== "fresh")
      throw new Error("Expected a fresh fixture snapshot.");
    expect(
      refreshed.snapshot.projects.find(
        (item) => item.project_id === "project-fixture-1",
      )?.name,
    ).toBe(projectA.name);
  });
  ...
  it("selects a native folder through an opaque snapshot reference without fake sync", async () => {
    ...
    expect(selectSource).toHaveBeenCalledWith(
      expect.objectContaining({ projectId: "project-fixture-1" }),
    );
  ...
  it("keeps the source and dirty state when the native picker is cancelled", async () => {
    ...
    await clickAria("Project settings");
    setInput("Project name", "Dirty draft after cancellation");
    expect(button("Save").disabled).toBe(false);
    await clickButton("Folder snapshot");
    ...
    expect(
      labelledControl<HTMLInputElement>("Project name", "input").value,
    ).toBe("Dirty draft after cancellation");
  ...
  it("allows only one native picker request while selection is in flight", async () => {
    ...
    await clickButton("Save");
    await clickAria("Project settings");
    setInput("Project name", "Controls locked during selection");

    const pending = deferred<ProjectSourceV1>();
    const selectSource = vi
      .spyOn(provider, "selectProjectSource")
    ...
    await act(async () =>
      pending.resolve({
        ...initialSource,
        display_name: "Replacement research folder",
      }),
    );
    await flush();
  ...
  it("cancels an in-flight native ingest and closes without waiting for its original promise", async () => {
    ...
    const selectSource = vi
      .spyOn(provider, "selectProjectSource")
      .mockImplementation(() => pending.promise);
    const cancelSource = vi
      .spyOn(provider, "cancelProjectSource")
      .mockImplementation(async () => {
        pending.reject({
          code: "workspace_selection_cancelled",
          message: "No research folder was selected.",
    ...
    await clickAria("Project settings");
    setInput("Project name", "Cancel in-flight snapshot");
  ...
  it("ignores a picker completion after a close request and a later selection wins", async () => {
    ...
    const selectSource = vi
      ...
      .mockResolvedValueOnce({
        ...selected,
        display_name: "Current research folder",
      });
    const settleSource = vi.spyOn(provider, "settleProjectSource");
    ...
    await clickAria("Project settings");
    setInput("Project name", "Editing after invalidating picker");
    const folderButton = button("Folder snapshot");
    await clickElement(folderButton);
    await clickAria("Close settings");
    expect(screenText()).toContain("Discard unsaved changes?");
    expect(cancelSource).toHaveBeenCalledWith(
      selectSource.mock.calls[0]?.[0].actionId,
    );
    ...
    expect(
      labelledControl<HTMLInputElement>("Project name", "input").value,
    ).toBe("Editing after invalidating picker");
    expect(screenText()).toContain("New workspace");
    expect(document.querySelector(".form-error")).toBeNull();

    await act(async () =>
      first.resolve({ ...selected, display_name: "Stale research folder" }),
    );
    await flush();
  ...
  it("keeps dirty drawer drafts until Escape, overlay, or close is confirmed", async () => {
    ...
    opener.focus();
    await clickAria("Project settings");
    setInput("Project name", "A retained project draft");
    provider.failNextProjectSaveWithUnknownError();
    await clickButton("Save");

    expect(
      labelledControl<HTMLInputElement>("Project name", "input").value,
    ).toBe("A retained project draft");
    ...
    const projectName = labelledControl<HTMLInputElement>(
      "Project name",
      "input",
    );
    await act(async () => projectName.focus());
    expect(alertDialog?.contains(document.activeElement)).toBe(true);
    await clickButton("Keep editing");
...
async function renderProduct(
  fixture: FixtureDesktopProductProvider,
): Promise<Root> {
  const container = document.createElement("div");
  document.body.appendChild(container);
...
function projectOptionValue(
  switcher: HTMLSelectElement,
  projectId: string,
): string {
...
function button(label: string): HTMLButtonElement {
  ...
  const matches = exact.length
    ? exact
    : all.filter((item) => item.textContent?.trim().startsWith(label));
...
function setCheckbox(label: string, checked: boolean): void {
  const control = labelledControl<HTMLInputElement>(
    label,
    'input[type="checkbox"]',
  );
  act(() => {
...
function withAdvancedRetry(
  ...
  return {
    ...
    runs: snapshot.runs.map((run) =>
      run.id === queuedRun.id ? queuedRun : run,
    ),
  };
}

function withTwoAdvancedRetries(
  snapshot: DesktopProductSnapshot,
): DesktopProductSnapshot {
  const once = withAdvancedRetry(snapshot);
  const first = once.runs.find((run) => run.id === "run-failed-model");
  if (!first?.current_attempt)
    throw new Error("Expected the first retry attempt.");
  const failedFirstRetry = {
    ...first.current_attempt,
    status: "failed" as const,
    queued_reason: null,
    error:
      snapshot.runs.find((run) => run.id === first.id)?.current_error ?? null,
...
function withRunOutputIdentity(
  ...
  if (attemptId !== null && !sourceAttempt)
