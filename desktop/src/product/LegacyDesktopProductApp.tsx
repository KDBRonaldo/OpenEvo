...
type RevisionRefV1 = NonNullable<
  NonNullable<ProjectV1["remote"]>["active_revision"]
>;
type CodexReasoningEffort = NonNullable<
  ProjectV1["execution"]["reasoning_effort"]
>;
...
type ProjectSelection =
  | {
      ...
      readonly sampleProjectId: SampleScientificProjectId;
    }
...
type ProfileSaveAttemptResult = {
  readonly saved: boolean;
  readonly pendingIntent: ProfileSaveIntent | null;
};
...
export interface LegacyDesktopProductAppProps {
...
export function LegacyDesktopProductApp({
  ...
  const [openSessionRunId, setOpenSessionRunId] = useState<string | null>(null);
  const [sessionTasksByDigest, setSessionTasksByDigest] = useState<
    Readonly<Record<string, ProjectV1["task"]>>
  >({});
  const [requestedProjectHead, setRequestedProjectHead] =
    useState<RevisionRefV1 | null>(null);
  ...
  const [openConnectionAfterRefresh, setOpenConnectionAfterRefresh] =
    useState(false);
  ...
  const pendingProjectActivation = useRef<PendingProjectActivation | null>(
    null,
  );
  const pendingRunRetry = useRef<PendingRunRetry | null>(null);
  const [pendingRetryPoll, setPendingRetryPoll] =
    useState<PendingRunRetry | null>(null);
  ...
  if (refreshCoordinator.current === null)
    refreshCoordinator.current = new SnapshotRefreshCoordinator();
  ...
  const reportInitialSnapshotFailure = useCallback(
    (error: unknown): void => {
      if (
        authoritativeSnapshotPublished.current ||
        initialSnapshotFailureReported.current
      )
        return;
  ...
  const publishRefresh = useCallback(
    (publication: SnapshotRefreshPublication): void => {
        ...
        setSnapshot((current) =>
          current
            ? {
                ...
                stream: {
                  status: "stale",
                  epoch: publication.epoch ?? current.stream.epoch,
          ...
          current
            ? {
                ...
                stream: {
                  ...
                  error:
                    publication.error instanceof DesktopApiError
                      ? publication.error.apiError
                      : null,
                },
              }
            : current,
      ...
      if (result.status !== "fresh") {
        ...
        if (result.status === "error")
          reportInitialSnapshotFailure(resultError);
        setSnapshot((current) =>
          current ? { ...current, stream: result.stream } : current,
        );
        if (result.status === "error") {
      ...
      if (!retryRecoveryReadFailed && !pendingRetry && recoveredRetry) {
        ...
        if (!pendingRetry.acceptedRun) {
          setActionError({
            owner: errorOwner,
            message:
              "The retry outcome is not yet confirmed. OpenEvo will keep checking the remote session.",
            selectionIdentity: selectionIdentityRef.current,
          });
      ...
      if (
        pendingRetry?.transportSettled &&
        retryAdvancedInSnapshot(next, pendingRetry)
      ) {
        clearPendingRetry(pendingRetry);
      } else if (pendingRetry?.acceptedRun) {
        next = mergeAuthoritativeRetryRun(
          next,
          pendingRetry.acceptedRun,
          pendingRetry,
        );
      }
      ...
      setProjectSelection((current) => {
        ...
        if (
          ...
          next.projects.some(
            (project) => project.project_id === current.projectId,
          )
        ) {
          return current;
        }
        if (
          ...
          next.profiles.some(
            (profile) => profile.profile_id === current.profileId,
          )
        ) {
          return current;
        }
        const projectId =
          next.state.active_project?.project_id ?? next.projects[0]?.project_id;
  ...
  const refresh = useCallback(
    (
      ...
      isRelevant: SnapshotRefreshGuard = () => true,
  ...
  const profile = viewingSample
    ...
    : project
      ? (snapshot?.profiles.find(
          (item) => item.profile_id === project.profile_id,
        ) ?? null)
      : selectedWorkspaceProfileId
        ? (snapshot?.profiles.find(
            (item) => item.profile_id === selectedWorkspaceProfileId,
          ) ?? null)
        : (snapshot?.profiles[0] ?? null);
  const connectionSettingsProfile =
  ...
  useEffect(() => {
    setRequestedProjectHead(null);
    setOpenSessionRunId(null);
  }, [coreProjectId]);
  const projectRuns = stableRunOrder(
    snapshot?.runs.filter((run) => run.project_id === coreProjectId) ?? [],
  );
  useEffect(() => {
    if (!project) return;
    setSessionTasksByDigest((current) => ({
      ...current,
      [taskDigest(project.task)]: project.task,
    }));
  }, [project?.project_id, project?.task.title, project?.task.objective]);
  const projectArtifacts =
    snapshot?.artifacts.filter(
      (artifact) => artifact.project_id === coreProjectId,
    ) ?? [];
  const activeRun = projectRuns.find((run) => !isTerminal(run.status)) ?? null;
  const projectSessionReady = snapshot
    ? hasReadySelectedProjectSession(snapshot, project)
    : false;
  const runPollingIdentity = useMemo(
    () =>
      runPollingIdentityFor(
        provider,
        snapshot,
        project,
        activeRun,
    ...
    async (
      ...
      try {
        ...
        if (
          ...
          [409, 410, 412].includes(error.apiError.http_status)
        ) {
        ...
        if (actionStateGeneration.current === stateOwner)
          setActionState("idle");
      }
    },
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      if (
        !active ||
        pendingRunRetry.current !== pendingRetryPoll ||
        attempts >= PENDING_RETRY_REFRESH_LIMIT
      )
        return;
      attempts += 1;
      await refresh(
        "poll",
        () => active && pendingRunRetry.current === pendingRetryPoll,
      );
      if (
        active &&
        pendingRunRetry.current === pendingRetryPoll &&
  ...
  useEffect(() => {
    ...
    const setupProjectId =
      snapshot.activeOperation?.operation_kind === "project_activate"
        ? snapshot.activeOperation.resource.resource_id
        : (snapshot.state.active_project?.project_id ?? null);
    const setupProject = snapshot.projects.find(
      (item) => item.project_id === setupProjectId,
    );
    if (
      !setupProject ||
      setupProject.evolution_configuration_state !== "pending"
    )
      return;
  ...
  useEffect(() => {
    if (!snapshot || (!openConnectionSettings && !openConnectionAfterRefresh))
      return;
  ...
  if (!snapshot) {
    return (
      <InitialSnapshotSample
        workspace={workspace}
        state={
          loadError ? { kind: "failed", error: loadError } : { kind: "pending" }
        }
  ...
  const displayedConnectionState =
    profile && connection.profile_id !== profile.profile_id
      ? "disconnected"
      : connection.state;
  const settingsProject = creatingProject ? null : project;
  const settingsFormIdentity = settingsProject
    ? `project:${settingsProject.project_id}`
    : "create";
  ...
  const profilesWithoutProjects = snapshot.profiles.filter(
    (candidate) =>
      !snapshot.projects.some(
        (candidateProject) =>
          candidateProject.profile_id === candidate.profile_id,
      ),
  );
  const selectedOperation =
    ...
    operationBelongsToSelection(snapshot.activeOperation, project, profile)
      ? snapshot.activeOperation
      : null;
  const activeOperationRunning =
    snapshot.activeOperation !== null &&
    !["succeeded", "failed", "cancelled"].includes(
      snapshot.activeOperation.state,
    );
  const recoveredMaintenanceBusy =
    activeOperationRunning &&
    selectedOperation !== null &&
    ["project_doctor", "project_repair"].includes(
      selectedOperation.operation_kind,
    );
  const maintenanceBusy = localMaintenanceBusy || recoveredMaintenanceBusy;
  const canCreateProject =
    profile?.connection_state === "connected" && !maintenanceBusy;
  const selectedOperationCanCancel =
    selectedOperation !== null &&
    !["project_doctor", "project_repair"].includes(
      selectedOperation.operation_kind,
    );
  ...
  const coreCanActivateProject =
    connection.profile_id === profile?.profile_id &&
    (displayedConnectionState === "online" ||
      displayedConnectionState === "degraded" ||
      (connection.state === "offline" &&
        connection.failure?.code === "core_not_started"));
  const canShowActivation =
    coreCanActivateProject &&
  ...
  const retryFailedRun = (run: RunV1): void => {
    ...
    if (!retryRun) {
      void act(() =>
        Promise.reject(new DesktopProductProviderUnavailableError()),
      );
      return;
    }
    const existing = pendingRunRetry.current;
    const errorOwner = reserveActionErrorOwner();
    const sameUnprovenRetry =
      existing?.runId === run.id && !retryAdvancedRun(run, existing);
    ...
    void act(
      ...
      if (
        ...
        retryAdvancedRun(acceptedRetryResponse, pending)
      ) {
        pending.acceptedRun = acceptedRetryResponse;
        setSnapshot((current) =>
          current
            ? mergeAuthoritativeRetryRun(
                current,
                acceptedRetryResponse,
                pending,
              )
        ...
        result.refreshedSnapshot &&
        retryAdvancedInSnapshot(result.refreshedSnapshot, pending)
      ) {
        clearPendingRetry(pending);
      } else if (
        result.error === null ||
        isAmbiguousRetryOutcome(result.error)
      ) {
  ...
  return (
    <div
      ...
      data-system-maintenance-available={String(
        provider.systemMaintenanceAvailable,
      )}
    >
      <aside className="product-sidebar" aria-label="Primary navigation">
        ...
        <nav className="product-nav">
          ...
          />
          {!viewingSample && project && projectRuns.length > 0 ? (
            <div className="sidebar-sessions" aria-label={`${project.name} sessions`}>
              <div className="sidebar-sessions-label">Sessions</div>
              {projectRuns.map((run) => {
                const title = sidebarSessionTitle(
                  project,
                  run,
                  projectRuns,
                  sessionTasksByDigest,
                );
                return (
                  <button
                    type="button"
                    className={`sidebar-session-item ${
                      workspace === "research" && openSessionRunId === run.id
                        ? "active"
                        : ""
                    }`}
                    key={run.id}
        ...
        <div className="sidebar-foot">
          <div className="sidebar-foot-label">
            {viewingSample ? "Demo progress" : "Current Project Head"}
          </div>
          <div className="sidebar-revision">
            <CircleDot size={15} />
            <span>
              {viewingSample
                ? `Generation ${selectedSampleProject.activeProjectHeadGeneration}`
                : revisionLabel(project, projectRuns)}
            </span>
          </div>
        </div>
      </aside>
        ...
        <header className="product-topbar">
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  if (selectedSample) {
                    setProjectSelection({
                      kind: "sample",
                      sampleProjectId: selectedSample.id,
                    });
                    return;
                  }
          ...
          <div className="topbar-actions">
            {viewingSample || !profile ? (
              ...
              <>
                <ConnectionBadge
                  state={displayedConnectionState}
                  profileName={profile.name}
                />
                <IconButton
                  label="Remote workspace settings"
                  onClick={() =>
                    setConnectionSettingsMode({
                      kind: "edit",
        ...
        <main className="product-main">
          {viewingSample ? (
            <SampleScientificProjectView
              ...
              onConnectRemote={() =>
                setConnectionSettingsMode({ kind: "create" })
              }
            />
          ) : null}
          {!connectionSettingsMode && visibleActionError ? (
            <InlineNotice
              tone="error"
              ...
              actionLabel={
                actionRecovery?.kind === "readmit_run"
                  ? "Re-admit session"
                  : undefined
              }
              onAction={
                actionRecovery?.kind === "readmit_run" &&
                project &&
                actionRecovery.projectId === project.project_id
                  ? () =>
                      void act(
                        () =>
                          provider.startRun({
          ...
          !["succeeded", "failed", "cancelled"].includes(
            ...
            <section className="operation-cancel-bar" aria-live="polite">
              <div>
                ...
                <span>
                  {selectedOperation.progress?.label ??
              ...
              {selectedOperationCanCancel ? (
                <button
                  ...
                  disabled={cancellingOperation || maintenanceBusy}
                >
                  <Square size={14} />{" "}
                  {cancellingOperation ? "Cancelling..." : "Cancel operation"}
                </button>
              ) : null}
            </section>
          ) : null}
              ...
              onConnect={(selectedProfile) =>
                void act(() =>
                  provider.connectProfile(
                    ...
                    resourceIntent(snapshot, selectedProfile.etag),
                  ),
                )
              }
              onAccept={(profileId) => {
          ...
          servicesNeedAttention ? (
            <div className="service-health-notice" role="status">
              <AlertCircle size={17} />
              <div>
                <strong>Remote services need attention</strong>
                <span>
                  Review the remote environment before starting another session.
                </span>
              </div>
              <button
                type="button"
            ...
            <ResearchWorkspace
              ...
              executionModeLabel={
                project
                  ? executionModeCapability(
                      snapshot.executionModeCapabilities,
              ...
              modelService={
                projectServices.find(
                  (service) => service.kind === "inference",
                ) ?? null
              }
              canStart={canStart}
              ...
              onStart={(task) =>
                ...
                void act(
                  async () => {
                    const taskChanged =
                      task.title !== project.task.title ||
                      task.objective !== project.task.objective;
                    const runProject = taskChanged
                      ...
                      : project;
                    return provider.startRun({
                      ...resourceIntent(snapshot, runProject.etag),
                      projectId: runProject.project_id,
                    });
                  },
                  { kind: "readmit_run", projectId: project.project_id },
                )
              }
              ...
              onCancel={() =>
                ...
                void act(() =>
                  provider.cancelRun(
                    activeRun.id,
                    resourceIntent(snapshot, activeRun.etag),
                  ),
                )
              }
              onOpenSettings={() => {
                setCreatingProject(project === null);
                setSettingsOpen(true);
              }}
              onOpenConnection={() =>
                setConnectionSettingsMode(
              ...
              conversationRunId={openSessionRunId}
              onOpenConversation={setOpenSessionRunId}
              sessionTasksByDigest={sessionTasksByDigest}
              onRememberSessionTask={(task) =>
                setSessionTasksByDigest((current) => ({
                  ...current,
                  [taskDigest(task)]: task,
                }))
              }
            />
          ) : null}
      ...
      {settingsOpen ? (
        <ProjectConfigurationDialog
          ...
          variant={creatingProject ? "create" : "edit"}
          ...
          onSave={async (input, actionId, pendingSourceActionId) => {
            ...
            let pendingSourceOutcome: SaveAttemptResult["pendingSourceOutcome"] =
              null;
            const result = await act(async () => {
              if (requestProject) {
                ...
                if (pendingSourceActionId) {
                  await provider.settleProjectSource(
                    pendingSourceActionId,
                    "adopt",
                  );
                ...
                if (
                  ...
                  const updated = afterUpdate?.projects.find(
                    (item) => item.project_id === requestProject.project_id,
                  );
                  if (!afterUpdate || !updated) {
                    throw new DesktopProductUserError(
                      "The prepared project is not available for activation. Refresh and try again.",
                    );
                  }
                ...
                if (!profile)
                  throw new DesktopProductUserError(
                    "Add a remote workspace before creating a project.",
                  );
                ...
                if (!pending) {
                  const created = await provider.createProject(
                    {
                      ...
                      execution:
                        input.execution ??
                        subscriptionExecution(DEFAULT_CODEX_MODEL),
                  ...
                  pending = {
                    ...
                    activationActionId: `${actionId}-activate`,
                  };
                ...
                const current = afterCreate?.projects.find(
                  (item) => item.project_id === pending.projectId,
                );
                if (!afterCreate || !current) {
                  throw new DesktopProductUserError(
                    "The new project is not available for activation. Refresh and try again.",
                  );
                }
                await provider.activateProject(
                  current.project_id,
            ...
            return {
              saved: result.saved,
              replaceActionId: requestPreconditionChanged(
                result,
                requestEpoch,
                requestProject
                  ? {
                      kind: "project",
                      id: requestProject.project_id,
                      etag: requestProject.etag,
                    }
                  : null,
              ),
          ...
          onSelectSource={(actionId) =>
            provider.selectProjectSource({
              ...mutationIntent(snapshot, actionId),
          ...
          onSettleSource={(actionId, outcome) =>
            provider.settleProjectSource(actionId, outcome)
          }
        />
      ) : null}
      {connectionSettingsMode &&
      (connectionSettingsMode.kind === "create" ||
        connectionSettingsProfile) ? (
        <RemoteWorkspaceDrawer
          profile={connectionSettingsProfile}
          observedProfiles={snapshot.profiles}
          streamEpoch={
            snapshot.stream.status === "fresh" ? snapshot.stream.epoch : null
          }
          busy={lifecycleMutationBusy}
          ...
          onSave={async (intent) => {
            ...
            const confirmedProfileId =
              route.kind === "create"
                ? (returnedProfile.current?.profile_id ??
                  route.confirmedProfileId)
                : null;
            const createdProfile = confirmedProfileId
              ...
              : null;
            const saveConfirmed =
              route.kind === "create" ? createdProfile !== null : result.saved;
            if (saveConfirmed) {
              setActionError(null);
              setConnectionSettingsMode(null);
              if (createdProfile) {
            ...
            const resource =
              route.kind === "update"
                ? {
                    ...
                    id: route.profileId,
                    etag: route.intent.etag,
                  }
                : null;
            return {
              saved: false,
              pendingIntent: requestPreconditionChanged(
                result,
                requestEpoch,
                resource,
              )
      ...
      <aside className="product-sidebar" aria-label="Primary navigation">
        ...
        <nav className="product-nav">
          ...
          <NavButton
            ...
            label="Evolution"
            active={workspace === "evolution"}
            onClick={() => onWorkspaceChange("evolution")}
          />
          <NavButton
            icon={Activity}
            label="System"
            active={workspace === "system"}
            onClick={() => onWorkspaceChange("system")}
          />
        </nav>
        <div className="sidebar-foot">
              ...
              <select
                ...
                onChange={(event) => {
                  const selected = SAMPLE_SCIENTIFIC_PROJECTS.find(
                    (sample) =>
                      sampleOptionKey(sample.id) === event.target.value,
                  );
...
class SnapshotRefreshCoordinator {
  ...
  private publish: ((publication: SnapshotRefreshPublication) => void) | null =
    null;
  ...
  request(
    ...
    if (!this.mounted || this.provider !== provider)
      return Promise.resolve(null);
    this.nextWatermark += 1;
    const watermark = this.nextWatermark;
    return new Promise((resolve) => {
      this.queuedBatch(provider, watermark).waiters.push({
        source,
        watermark,
        isRelevant,
        resolve,
  ...
  private async execute(batch: SnapshotRefreshBatch): Promise<void> {
    ...
    const relevance = batch.waiters.map((waiter) =>
      safeRefreshGuard(waiter.isRelevant),
    );
    if (!relevance.some(Boolean)) {
      this.resolveBatch(batch, null);
      this.publish({
        kind: "pending",
        epoch: refreshPublicationEpoch(publication),
      });
  ...
  private hasTrailingAfter(batch: SnapshotRefreshBatch): boolean {
    return (
      this.trailing?.provider === batch.provider &&
...
const SNAPSHOT_REFRESH_SOURCE_PRIORITY: Readonly<
  Record<SnapshotRefreshSource, number>
> = {
  mutation: 6,
  reconcile: 5,
  manual: 4,
...
function orderedRefreshWaiters(
  waiters: readonly SnapshotRefreshWaiter[],
): SnapshotRefreshWaiter[] {
  return [...waiters].sort(
    (left, right) =>
      SNAPSHOT_REFRESH_SOURCE_PRIORITY[right.source] -
        SNAPSHOT_REFRESH_SOURCE_PRIORITY[left.source] ||
      left.watermark - right.watermark,
  );
}

function refreshPublicationEpoch(
  publication: SnapshotRefreshPublication,
): number | null {
...
function useActiveRunPolling(
  ...
  useEffect(() => {
    ...
    const isCurrent = () =>
      !cancelled && sameRunPollingIdentity(currentIdentity.current, identity);
    const schedule = () => {
      if (!isCurrent()) return;
      timer = setTimeout(() => {
        timer = null;
...
function snapshotHasRunPollingIdentity(
  ...
  const project =
    snapshot.projects.find(
      (item) => item.project_id === expected.desktopProjectId,
    ) ?? null;
  const activeProject = snapshot.state.active_project;
  if (
    !project ||
...
function NavButton({
  ...
  return (
    <button
      type="button"
      className={`product-nav-item ${active ? "active" : ""}`}
...
function ConnectionBadge({
  ...
  state: DesktopProductSnapshot["state"]["core"]["state"];
  profileName: string;
}) {
  const tone =
    state === "online"
  ...
  return (
    <div
      className={`connection-badge ${tone}`}
      title={`${profileName}: ${connectionLabel(state)}`}
    >
      {isConnectionBusy(state) ? (
...
function ConnectionGate({
  ...
  if (coreBelongsToSelectedProfile && core.state === "degraded") {
    return (
      <InlineNotice
        tone="warning"
        title="Remote workspace needs attention"
        detail={
          core.failure?.message ??
          "Open System to review service status and operation logs."
        }
      />
    );
  }
    ...
    return (
      <section className="connection-gate host-review" aria-live="polite">
        ...
        <div className="gate-copy">
          ...
          <p>
            Compare this fingerprint with the value provided by your server
            administrator.
          </p>
        ...
        <button
          ...
          title={
            busy
              ? "A connection action is already running"
  ...
  if (coreBelongsToSelectedProfile && isConnectionBusy(core.state)) {
    ...
    return (
      <section className="connection-gate" aria-live="polite">
        <div className="gate-icon progress">
          <LoaderCircle className="spin" size={21} />
        </div>
        <div className="gate-copy">
          <h2>{connectionHeading(core.state)}</h2>
          <p>
            {progress?.label ?? "Preparing the remote research environment."}
          </p>
          {progress ? (
  ...
  return (
    <section className="connection-gate" aria-live="polite">
      ...
      <div className="gate-copy">
        <h2>
          {core.state === "online"
            ? "Switch remote workspace"
            : "Remote workspace is offline"}
        </h2>
        <p>
          {credentialReason ??
            (core.state === "online"
              ? "Connect this project's assigned workspace before activating or running it."
              : coreBelongsToSelectedProfile
                ? (core.failure?.message ??
                  "Connect to run research sessions and inspect evolution.")
                : "Connect this remote workspace to continue.")}
        </p>
      </div>
      <button
        className="primary-button"
        type="button"
        onClick={() => (credentialReason ? onSetup() : onConnect(profile))}
        disabled={busy}
        title={
          busy
            ? "A connection action is already running"
      ...
      >
        {credentialReason ? <Settings size={16} /> : <ArrowRight size={16} />}{" "}
        {credentialReason ? "Configure" : "Connect"}
      </button>
    </section>
  );
}
  ...
  return (
    <section className="connection-gate" aria-live="polite">
      ...
      <div className="gate-copy">
        ...
        <p>
          {blocked
            ? "This project cannot be activated by this Desktop release. Review the remote Core state before retrying."
            : "Activation binds this project to its own profile and tunnel."}
        </p>
      </div>
      {!blocked ? (
        <button
          ...
          title={disabledReason ?? "Activate this project"}
        >
          <ArrowRight size={16} /> Activate project
        </button>
      ) : null}
    </section>
  );
...
function ResearchWorkspace({
  ...
  onOpenConversation: (runId: string | null) => void;
  sessionTasksByDigest: Readonly<Record<string, ProjectV1["task"]>>;
  onRememberSessionTask: (task: ProjectV1["task"]) => void;
}) {
  const [selectedRunId, setSelectedRunId] = useState<string | null>(null);
  const [taskDraftTitle, setTaskDraftTitle] = useState(
    project?.task.title ?? "Research task",
  );
  const [taskDraftObjective, setTaskDraftObjective] = useState(
    project?.task.objective ?? "",
  );
  useEffect(() => {
    setSelectedRunId(null);
    onOpenConversation(null);
    setTaskDraftTitle(project?.task.title ?? "Research task");
    setTaskDraftObjective(project?.task.objective ?? "");
  }, [project?.project_id, onOpenConversation]);
  useEffect(() => {
  ...
  if (!project) {
    if (!hasProfile)
      return (
        <EmptyState
          icon={PanelLeft}
          title="Add a remote workspace"
          detail="Enter the server that will run research sessions."
          action="Add remote workspace"
          actionIcon={Plus}
    ...
    if (!canCreateProject)
      return (
        <EmptyState
          icon={PanelLeft}
          title="Connect the remote workspace"
          detail="Confirm the server connection before creating a research project."
        />
      );
    return (
      <EmptyState
        ...
        detail="Define a task and source to begin a session."
  ...
  const conversationRun =
    conversationRunId === null
      ? null
      : (runs.find((run) => run.id === conversationRunId) ?? null);
  const taskDraftValid =
    taskDraftTitle.trim().length > 0 && taskDraftObjective.trim().length > 0;
  const taskStartReason = !taskDraftValid
    ? "Enter a title and instructions for the next task."
    : startReason;
  const evolutionOutputsLoading = artifactCollection.status !== "complete";
  const observedOutputProjectHead =
    observedRun && !evolutionOutputsLoading
  ...
  const recover = (run: RunV1) => {
    ...
    if (
      action === "openevo_can_install" ||
      action === "openevo_can_reconfigure" ||
      action === "unsupported"
    )
      return { label: "Open System", onClick: onOpenSystem, disabled };
    if (action === "user_action_required")
      return { label: "Edit project", onClick: onOpenSettings, disabled };
  ...
  if (conversationRun) {
    return (
      <SessionConversation
        run={conversationRun}
        runs={runs}
        task={taskContentForRun(project, conversationRun, sessionTasksByDigest)}
        provider={provider}
    ...
    <div className="workspace-stack" data-testid="research-workspace">
      <div className="workspace-heading">
        <div>
          ...
          <h1>{project.name}</h1>
          <p>Prepare one task at a time against the current Project Head.</p>
        </div>
          ...
          <button
            ...
            onClick={() => {
              const task = {
                ...
                objective: taskDraftObjective.trim(),
              };
              onRememberSessionTask(task);
              onStart(task);
            }}
            disabled={!canStart || !taskDraftValid}
            title={taskStartReason ?? "Start a new research session"}
          >
            <Play size={16} fill="currentColor" /> Start session
          </button>
        </div>
      </div>
      {taskStartReason && !activeRun ? (
        <div className="disabled-reason">
          <AlertCircle size={14} /> <span>{taskStartReason}</span>
          {taskStartReason.startsWith("Refresh") ? (
      ...
      <div className="research-grid">
        <section className="product-panel task-panel">
          <div className="panel-heading">
            <div>
              <span className="panel-kicker">Task draft</span>
              <h2>What should the agent do next?</h2>
            </div>
            <span className="source-chip">
              <FolderOpen size={14} /> {project.source.display_name}
            </span>
          </div>
          <div className="next-task-fields">
            <label>
              ...
              <input
                ...
                disabled={activeRun !== null || busy}
                onChange={(event) => setTaskDraftTitle(event.target.value)}
                placeholder="Give this task a short name"
              />
            </label>
            <label>
              Task instructions
              <textarea
                rows={6}
                maxLength={65_536}
                value={taskDraftObjective}
                disabled={activeRun !== null || busy}
                onChange={(event) => setTaskDraftObjective(event.target.value)}
                placeholder="Describe the concrete result the agent should produce in this session."
              />
            </label>
            <p className="form-help">
              Starting the session saves these instructions and runs them as the
              next task.
            </p>
          </div>
          <div className="brief-footer">
            ...
            <div>
              ...
              <strong>
                {
                  Object.values(project.evolution.targets).filter(
        ...
        <section className="product-panel active-run-panel">
          <div className="panel-heading">
            <div>
              ...
              <h2>
                {observedRun
                  ? sessionTitle(observedRun, runs)
                  : "No session selected"}
              </h2>
            </div>
            {observedRun ? (
              <StatePill state={observedRun.status} />
            ) : (
              <span className="muted-pill">Ready</span>
            )}
          </div>
          {observedRun === activeRun && activeRun ? (
            <>
              ...
              <button
                ...
                title={
                  busy ? "Another action is running" : "Cancel this session"
                }
              >
            ...
            <>
              <RunOutcomeSummary
                run={observedRun}
                isLatest={observedRun.id === latestTerminal?.id}
                outputProjectHead={observedOutputProjectHead}
                evolutionOutputsLoading={evolutionOutputsLoading}
                onOpenEvolution={onOpenEvolution}
                recovery={recover(observedRun)}
              />
            ...
            <div className="quiet-empty">
              <Play size={22} />
              <p>Start a session when the remote workspace is ready.</p>
            </div>
          )}
        </section>
      </div>
      ...
      {evolutionOutputsLoading ? (
        <div className="disabled-reason" role="status">
          <RefreshCw size={14} />
          <span>
            Artifact collection is incomplete. Refetch all artifact pages before
            identifying session evolution outputs.
          </span>
          <button type="button" className="text-button" onClick={onRefresh}>
            <RefreshCw size={14} /> Refetch artifacts
          </button>
        </div>
      ) : null}
      ...
      <section className="history-section">
        ...
        {runs.length ? (
          <SessionTable
            ...
            selectedRunId={observedRun?.id ?? null}
            modelService={modelService}
            onOpenRun={(runId) => {
  ...
  return (
    <div className="revision-pin">
      <div>
        ...
        <strong>
          {run.pinned_revision
            ? `Project Head ${run.pinned_revision.generation}`
            : "Admission pending"}
        </strong>
      </div>
      <ArrowRight size={16} />
      <div>
        ...
        <strong>
          {run.required_revision.relation === "active"
            ? "Active project head"
            : "Next-head transition"}
        </strong>
      </div>
      {run.revision_transition ? (
        <StatePill state={run.revision_transition.state} />
      ) : null}
    </div>
  );
}
...
function Timeline({
  ...
  entries: readonly DesktopProductSnapshot["timelines"][string][number][];
}) {
...
function SessionConversation({
  ...
  run: RunV1;
  runs: readonly RunV1[];
  task: ProjectV1["task"] | null;
  provider: DesktopProductProvider;
  streamEpoch: number;
  onBack: () => void;
}) {
  return (
    <div className="session-conversation-page">
      <header className="session-conversation-header">
        <button type="button" className="secondary-button" onClick={onBack}>
          <ArrowLeft size={15} /> Back to project
        </button>
        <div>
          <span className="panel-kicker">Session conversation</span>
          <h1>{task?.title ?? sessionTitle(run, runs)}</h1>
          <p>
            {sessionTitle(run, runs)} · {formatTime(run.created_at)}
          </p>
        </div>
        <StatePill state={run.status} />
      </header>
      <div className="session-conversation-authority">
        <span>
          {run.pinned_revision
            ? `Project Head ${run.pinned_revision.generation}`
            : "Admission pending"}
...
function SessionOutput({
  ...
  provider: DesktopProductProvider;
  streamEpoch: number;
  conversationTask?: ProjectV1["task"] | null;
}) {
  ...
  const filtered = logs.filter((entry) => {
    ...
    if (filter === "system")
      return entry.stream === "core" || entry.stream === "service";
    return entry.stream === filter;
  });
  const visible = filtered.slice(-200);
  return (
    <section
      className={`product-panel session-output-panel ${conversationTask !== undefined ? "conversation" : ""}`}
    ...
    >
        ...
        <div className="session-output-state">
          {loading ? (
            <LoaderCircle
              className="spin"
              size={14}
              aria-label="Refreshing session output"
            />
          ) : null}
      ...
      <div
        className="session-output-toolbar"
        role="group"
        aria-label="Session output filter"
      >
        {(
          [
            ["all", "All logs"],
          ...
          <article className="conversation-message user">
            ...
            {conversationTask ? (
              <div className="conversation-bubble">
                <strong>{conversationTask.title}</strong>
                <p>{conversationTask.objective}</p>
              </div>
            ) : (
              <div className="conversation-bubble unavailable">
                <p>
                  The backend did not return the historical instruction text for
                  this immutable Task Snapshot.
                </p>
              </div>
            )}
          </article>
      ...
      {error ? (
        ...
        <div
          className={`session-output-list ${conversationTask !== undefined ? "conversation-list" : ""}`}
          aria-live="polite"
        >
          {filtered.length > visible.length ? (
            <div className="session-output-limit">
              Showing the latest 200 matching records.
            </div>
          ) : null}
          {visible.map((entry) => (
            <article
            ...
            >
              <div className="session-output-meta">
                <span className={`session-stream ${entry.stream}`}>
                  {sessionStreamLabel(entry.stream)}
                </span>
        ...
        <div className="session-output-empty">
          No {filter === "all" ? "session" : filter} output has been reported.
        </div>
      )}
    </section>
  );
  ...
  return (
    <div className="session-table" role="table" aria-label="Session history">
      <div className="session-table-head" role="row">
        ...
        <span role="columnheader">Evolution output</span>
        <span role="columnheader">Updated</span>
      </div>
      {runs.map((run) => {
        const recovery = onRecover(run);
        const evolutionOutputsLoading =
        ...
        const sessionName = sessionTitle(run, runs);
        return (
          <div
            className={`session-table-row ${run.id === selectedRunId ? "selected" : ""}`}
            role="row"
            key={run.id}
          >
            <span role="cell">
              <button
                type="button"
                className="session-history-button session-open-button"
            ...
            <span role="cell" className="session-detail">
              <RunStatusText
                run={run}
                modelService={modelService}
                outputProjectHead={evolutionRevision}
                evolutionOutputsLoading={evolutionOutputsLoading}
              />
              {run.status === "queued" ? (
                <button
...
function taskDigest(task: ProjectV1["task"]): string {
  return sha256Utf8V2(canonicalJsonV2(task));
}

function taskContentForRun(
  project: ProjectV1,
  run: RunV1,
  remembered: Readonly<Record<string, ProjectV1["task"]>>,
): ProjectV1["task"] | null {
  const rememberedTask = remembered[run.task_snapshot.content_sha256];
  if (
    rememberedTask &&
    taskDigest(rememberedTask) === run.task_snapshot.content_sha256
  )
    return rememberedTask;
  return taskDigest(project.task) === run.task_snapshot.content_sha256
    ? project.task
    : null;
}
      ...
      <div>
        <strong>
          {run.status === "queued" && run.queued_reason
            ? queuedReasonLabel(run.queued_reason.code, modelService)
...
function RunStatusText({
  run,
  modelService,
  outputProjectHead = null,
  evolutionOutputsLoading = false,
}: {
  run: RunV1;
  modelService: ServiceV1 | null;
  outputProjectHead?: RevisionRefV1 | null;
  evolutionOutputsLoading?: boolean;
}) {
  if (run.status === "queued" && run.queued_reason) {
    const retry =
      run.queued_reason.retry_after_seconds === null
        ? ""
        : ` Check again in about ${run.queued_reason.retry_after_seconds} seconds.`;
    const model =
      run.queued_reason.code === "service_starting" &&
      modelService?.status === "starting"
        ? ` ${modelService.status_message ?? ""}`
        : "";
    return (
      <span>
        {run.queued_reason.summary}
        {model}
        {retry}
      </span>
    );
  }
  if (run.status === "failed" && run.current_error)
    return (
      <span>
        {run.current_error.message}
        {run.current_error.next_action
          ? ` ${run.current_error.next_action}`
          : ""}
      </span>
    );
  if (run.status === "cancelled")
    return <span>The session was cancelled before completion.</span>;
  if (run.status === "succeeded") {
    if (evolutionOutputsLoading)
      return (
        <span>
          Evolution output status is unavailable until all artifact pages are
          loaded.
        </span>
      );
    return (
      <span>
        {outputProjectHead
          ? `Project Head ${outputProjectHead.generation} contains this session's selected evolution outputs.`
          : "The session completed without evolution output artifacts."}
      </span>
    );
  }
...
function queuedReasonLabel(
  code: NonNullable<RunV1["queued_reason"]>["code"],
  modelService: ServiceV1 | null,
): string {
  ...
  return modelService?.status === "starting"
    ? "Model preparation"
    : "Service preparation";
}
...
function RunOutcomeSummary({
  ...
  evolutionOutputsLoading: boolean;
  onOpenEvolution: (projectHead: RevisionRefV1) => void;
  recovery: { label: string; onClick: () => void; disabled?: boolean };
}) {
  ...
  return (
    <div className={`completed-summary ${run.status}`}>
      ...
      <div>
        <strong>{`${subject} ${succeeded ? "complete" : run.status === "failed" ? "failed" : "cancelled"}`}</strong>
        <RunStatusText
          run={run}
          modelService={null}
          outputProjectHead={outputProjectHead}
          evolutionOutputsLoading={evolutionOutputsLoading}
        />
      </div>
      {succeeded && outputProjectHead ? (
        <button
          className="text-button"
          type="button"
          onClick={() => onOpenEvolution(outputProjectHead)}
        >
          View changes <ArrowRight size={14} />
        </button>
      ) : null}
      {run.status === "failed" ? (
        <button
...
function EvolutionWorkspace({
  ...
  artifacts: readonly ArtifactV1[];
  artifactCollection: ProductArtifactCollectionState;
  requestedProjectHead: RevisionRefV1 | null;
  provider: DesktopProductProvider;
  onRefresh: () => void;
  onOpenSettings: () => void;
}) {
  const activeRevision = project
    ? authoritativeActiveRevision(project, runs)
    : null;
  const historicalOutputHeads =
    artifactCollection.status === "complete"
      ? runs
          .map((run) => evolutionOutputRevisionForRun(artifacts, run))
          .filter((revision): revision is RevisionRefV1 => revision !== null)
      : [];
  const selectedProjectHead =
    requestedProjectHead &&
    historicalOutputHeads.some((revision) =>
      sameRevisionRef(revision, requestedProjectHead),
    )
      ? requestedProjectHead
      : activeRevision;
  const selectedProjectHeadIsActive =
    selectedProjectHead !== null &&
    activeRevision !== null &&
    sameRevisionRef(selectedProjectHead, activeRevision);
  const orderedArtifacts =
    selectedProjectHead && artifactCollection.status === "complete"
      ? selectedArtifactsForRevision(artifacts, selectedProjectHead)
      : [];
  const [selectedArtifactId, setSelectedArtifactId] = useState<string | null>(
    orderedArtifacts[0]?.id ?? null,
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (view === "content") {
        const result = await loadArtifactContentAfterPublication(
          provider,
          selectedArtifact.id,
        );
        const identityError = artifactContentIdentityError(
          selectedArtifact,
          result,
        );
        ...
        const identityError = artifactDiffIdentityError(
          selectedArtifact,
          result,
          artifacts,
        );
    ...
    void load()
      .catch((reason) => active && setError(userMessage(reason)))
  ...
  if (!project)
    return (
      <EmptyState
        ...
        detail="Choose a project to inspect revisions and artifacts."
      />
    );
  const selected =
  ...
  const evolutionEnabled = Object.values(project.evolution.targets).some(
    (target) => target.enabled,
  );
  return (
    <div className="workspace-stack" data-testid="evolution-workspace">
      <div className="workspace-heading">
        <div>
          ...
          <h1>Cross-session changes</h1>
          <p>
            Review what changed and which Project Head a session used or
            produced.
          </p>
        </div>
      </div>
      <section className="revision-strip">
        {selectedProjectHead ? (
          <div
            className={`revision-node ${selectedProjectHeadIsActive ? "active" : ""}`}
          >
            <span>
              {selectedProjectHeadIsActive
                ? "Active Project Head"
                : "Historical Project Head"}
            </span>
            <strong>Project Head {selectedProjectHead.generation}</strong>
            <small>
              {selectedProjectHeadIsActive
                ? "Used by the next session"
                : "Produced by the selected session"}
            </small>
          </div>
        ) : (
          <div className="revision-node">
            <span>Active Project Head</span>
            <strong>Project Head unknown</strong>
            <small>Waiting for an authoritative Project Head reference</small>
            <button type="button" className="text-button" onClick={onRefresh}>
              <RefreshCw size={13} /> Refetch Project Head
            </button>
          </div>
        )}
      </section>
      {!selectedProjectHead ? (
        <EmptyState
          icon={RefreshCw}
          title="Project Head relation is unknown"
          detail="Refetch before inspecting selected artifacts for a session."
          action="Refetch Project Head"
        ...
        <EmptyState
          icon={RefreshCw}
          title="Artifact collection is incomplete"
          detail="Refetch all artifact pages before inspecting revision membership."
          action="Refetch artifacts"
          actionIcon={RefreshCw}
          onAction={onRefresh}
        />
        ...
        evolutionEnabled ? (
          <EmptyState
            icon={MemoryStick}
            title="No evolved artifacts yet"
            detail="Complete a session to create memory, skills, and agent guidance for the next revision."
          />
        ) : (
          <EmptyState
            icon={Settings}
            title="Evolution is off"
            detail="No evolution targets are enabled for future sessions."
            action="Configure evolution"
            actionIcon={Settings}
            onAction={onOpenSettings}
          />
        )
            ...
            <div className="artifact-list-heading">
              <span>
                {selectedGeneration === null
                  ? "Project Head unknown"
                  : `Project Head ${selectedGeneration}`}
              </span>
              <strong>{orderedArtifacts.length} selected</strong>
            </div>
            {orderedArtifacts.map((artifact) => (
              <button
                key={artifact.id}
                type="button"
                className={`artifact-list-item ${artifact.id === selected?.id ? "active" : ""}`}
                onClick={() => setSelectedArtifactId(artifact.id)}
              >
                <span className={`artifact-icon ${artifact.artifact_type}`}>
            ...
            {selected ? (
              <>
                <div className="artifact-viewer-head">
                  <div>
                    <span className="panel-kicker">
                      {artifactTypeLabel(selected.artifact_type)}
                    </span>
                  ...
                  <div className="artifact-meta">
                    ...
                    {selected.scores[0] ? (
                      <span>
                        Quality {Math.round(selected.scores[0].value * 100)}%
                      </span>
                    ) : null}
                  </div>
                </div>
                ...
                >
                  <button
                    id="artifact-content-tab"
                    aria-controls="artifact-view-panel"
                    type="button"
                    role="tab"
                ...
                >
                  ...
                  {error ? (
                    <div className="artifact-load-error">
                      ...
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setArtifactLoadAttempt((attempt) => attempt + 1)
                        }
                      >
                  ...
                  {!loading && !error && view === "content" && content ? (
...
async function loadArtifactContentAfterPublication(
  ...
  for (
    let attempt = 0;
    attempt <= ARTIFACT_PUBLICATION_RETRY_DELAYS_MS.length;
    attempt += 1
  ) {
    try {
      ...
      await new Promise<void>((resolve) =>
        setTimeout(resolve, ARTIFACT_PUBLICATION_RETRY_DELAYS_MS[attempt]),
      );
    }
  }
...
function ArtifactContent({ content }: { content: ArtifactContentV1 }) {
  const [documentId, setDocumentId] = useState(
    content.documents[0]?.document_id ?? "",
  );
  useEffect(
    () => setDocumentId(content.documents[0]?.document_id ?? ""),
    [content.artifact_id],
  );
  const document =
    content.documents.find((item) => item.document_id === documentId) ??
    content.documents[0];
  const documentIndex = document
    ? content.documents.findIndex(
    ...
    >
      {content.truncated ? (
        <InlineNotice
          ...
          detail={`Showing ${content.documents.length} of ${content.total_documents} documents.`}
        />
      ) : null}
      {content.documents.length > 1 ? (
        <div
          ...
          role="tablist"
          aria-label="Artifact documents"
          onKeyDown={handleTablistKeyDown}
        >
          {content.documents.map((item, index) => (
            <button
              id={`artifact-document-tab-${index}`}
              aria-controls="artifact-document-panel"
              role="tab"
              aria-selected={item.document_id === document?.document_id}
              tabIndex={item.document_id === document?.document_id ? 0 : -1}
              key={item.document_id}
              type="button"
              className={
                item.document_id === document?.document_id ? "active" : ""
              }
              ...
              data-content-sha256={item.content_sha256}
              data-byte-size={item.byte_size}
              data-truncated={String(item.truncated)}
              onClick={() => setDocumentId(item.document_id)}
            >
              {item.display_name}
            </button>
          ))}
      ...
      {document ? (
        <pre
          id="artifact-document-panel"
          role={content.documents.length > 1 ? "tabpanel" : undefined}
          aria-labelledby={
            content.documents.length > 1
              ? `artifact-document-tab-${documentIndex}`
              : undefined
          }
          className="artifact-document"
          data-document-id={document.document_id}
          data-display-name={document.display_name}
          data-relative-path={document.relative_path ?? ""}
          data-mime-type={document.mime_type}
          data-content-sha256={document.content_sha256}
          data-byte-size={document.byte_size}
          data-truncated={String(document.truncated)}
        >
          {document.content}
        </pre>
      ) : null}
...
function ArtifactDiff({ diff }: { diff: ArtifactDiffV1 }) {
  if (diff.document_changes.length === 0)
    return (
      <div className="quiet-empty">
        <FileDiff size={22} />
        <p>No document changes are available for this revision.</p>
      </div>
    );
  return (
    <div
      ...
      data-previous-artifact-content-sha256={
        diff.previous_artifact_content_sha256
      }
    ...
    >
      {diff.truncated ? (
        <InlineNotice
          tone="warning"
          title="Change preview is truncated"
          detail="Some changes are not shown in this preview."
        />
      ) : null}
      {diff.document_changes.map((change, changeIndex) => {
        const oldPath =
          "old_document" in change ? change.old_document.relative_path : null;
        const newPath =
          "new_document" in change ? change.new_document.relative_path : null;
        const oldDocument =
          "old_document" in change ? change.old_document : null;
        const newDocument =
          "new_document" in change ? change.new_document : null;
        const heading =
          change.kind === "renamed"
            ? `${oldPath} to ${newPath}`
            : (newPath ?? oldPath);
        ...
        return (
          ...
          >
            ...
            {change.hunks.length === 0 ? (
              <p className="diff-document-empty">{emptyMessage}</p>
            ) : null}
          </section>
        );
      })}
...
function SystemWorkspace({
  ...
  const [confirmation, setConfirmation] = useState<SystemConfirmation | null>(
    null,
  );
  const [activity, setActivity] = useState<SystemActivity | null>(null);
  const [lastDoctorOperation, setLastDoctorOperation] =
    useState<LocalOperationV1 | null>(null);
  const [maintenanceError, setMaintenanceError] = useState<string | null>(null);
  const [desktopDiagnosticsStatus, setDesktopDiagnosticsStatus] = useState<
    string | null
  >(null);
  ...
  const runLocalOperation = async (
    ...
    try {
      ...
      for (
        let attempt = 0;
        !isLocalOperationTerminal(operation.state) &&
        attempt < SYSTEM_OPERATION_REFRESH_LIMIT;
        attempt += 1
      ) {
  ...
  const runCoreOperation = async (
    ...
    try {
      ...
      for (
        let attempt = 0;
        !isCoreOperationTerminal(operation.status) &&
        attempt < SYSTEM_OPERATION_REFRESH_LIMIT;
        attempt += 1
      ) {
        await waitForSystemRefresh();
  ...
  const runDiagnostics = async (): Promise<void> => {
    ...
    try {
      ...
      for (
        let attempt = 0;
        !isDiagnosticTerminal(report.status) &&
        attempt < SYSTEM_OPERATION_REFRESH_LIMIT;
        attempt += 1
      ) {
        await waitForSystemRefresh();
  ...
  const confirmAction = (): void => {
    ...
    if (pending.kind === "repair") {
      ...
      void runLocalOperation("Repair remote environment", () =>
    ...
    if (pending.kind === "restart") {
      void runCoreOperation(`Restart ${pending.service.display_name}`, () =>
    ...
    void runCoreOperation("Clean diagnostic history", () =>
      ...
      >
        <div className="workspace-heading">
          <div>
            ...
            <h1>Remote environment</h1>
            <p>Connection, service health, diagnostics, and recovery.</p>
          </div>
          {maintenanceAvailable ? (
            <div className="workspace-heading-actions">
              <button
                ...
                onClick={() => {
                  ...
                  void runLocalOperation("Check remote environment", () =>
          ...
          <section className="product-panel connection-panel">
            <div className="panel-heading">
              <div>
                ...
                <h2>{profile?.name ?? "No remote workspace"}</h2>
              </div>
              <StatePill state={displayedCoreState} />
            </div>
            <dl className="definition-list">
              ...
              <div>
                ...
                <dd>{displayedTunnel ? "Active" : "Not connected"}</dd>
              </div>
              <div>
                <dt>Compatibility</dt>
                <dd>
              ...
              <div>
                ...
                <dd>{projectSessionReady ? "Ready" : "Unavailable"}</dd>
              </div>
            </dl>
            <div className="system-button-row">
              ...
              >
                <Settings size={15} />{" "}
                {profile ? "Edit" : "Add remote workspace"}
              </button>
              {profile &&
              (displayedCoreState !== "online" || servicesNeedAttention) ? (
                <button
                  className="secondary-button"
                  type="button"
                  onClick={onConnect}
                  disabled={
                    busy ||
                    (profileOwnsCore && isConnectionBusy(core.state)) ||
                    missingCredentialReason(profile) !== null
                  }
                  title={
                    busy
                      ? "A connection action is already running"
                      : (missingCredentialReason(profile) ??
        ...
        <section className="services-section">
          <div className="section-heading">
            ...
            <div className="section-heading-actions">
              <span>
                {readyServices} of {services.length} ready
              </span>
              <button
                type="button"
                className="text-button"
          ...
          <div className="service-list">
            {services.map((service) => (
              <ServiceRow
                ...
                disabled={!canOperate}
                onRestart={
                  maintenanceAvailable && service.restartable
            ...
            {!services.length ? (
              <div className="empty-row">
                Services are unavailable for this project.
              </div>
            ) : null}
          </div>
        </section>
          ...
          <section className="services-section">
            ...
            <div className="maintenance-row">
              <div>
                ...
                <span>
                  Remove completed diagnostic reports older than 30 days.
                  Project inputs, runs, outputs, and active evolution artifacts
                  are retained.
                </span>
              </div>
              <button
        ...
        <section className="services-section">
          ...
          <div className="maintenance-row">
            <div>
              ...
              <span>
                Review or export private OpenEvo Desktop lifecycle and startup
                diagnostics.
              </span>
              {desktopDiagnosticsStatus ? (
                <span role="status">{desktopDiagnosticsStatus}</span>
              ) : null}
            </div>
            <div className="system-button-row">
              <button
  ...
  return (
    <div className="service-row">
      ...
      <div>
        ...
        <span>{service.status_message ?? stateLabel(service.status)}</span>
      </div>
    ...
    <section className="system-activity" aria-live="polite">
      ...
      {error ? (
        <div className="system-activity-error" role="alert">
          ...
          <span>
            <b>Next action:</b> {error.next_action}
          </span>
        </div>
      ) : null}
      {checks.length > 0 ? (
            ...
            <div key={check.id}>
              ...
              <div className="system-check-detail">
                ...
                {repairActionGuidance(check.repairAction) ? (
                  <small>
                    Next action: {repairActionGuidance(check.repairAction)}
                  </small>
                ) : null}
              </div>
            </div>
...
function SystemActivityStateIcon({ state }: { state: SystemActivityState }) {
  switch (state) {
    case "queued":
      return (
        <CircleDot className="system-activity-state-icon queued" size={16} />
      );
    case "running":
      return (
        <LoaderCircle
          className="system-activity-state-icon running spin"
          size={16}
        />
      );
    case "cancelling":
      return (
        <LoaderCircle
          className="system-activity-state-icon cancelling spin"
          size={16}
        />
      );
    case "succeeded":
      return (
        <CheckCircle2
          className="system-activity-state-icon succeeded"
          size={16}
        />
      );
    case "failed":
      return (
        <XCircle className="system-activity-state-icon failed" size={16} />
      );
    case "cancelled":
      return (
        <XCircle className="system-activity-state-icon cancelled" size={16} />
      );
    default:
...
function systemRepairAuthority(operation: LocalOperationV1 | null): {
  ...
  if (operation === null || operation.operation_kind !== "project_doctor") {
    return {
      enabled: false,
      title:
        "Run Check first so OpenEvo can identify supported repair actions.",
    };
  }
  if (!isLocalOperationTerminal(operation.state)) {
    return {
...
function repairActionGuidance(action: SystemRepairAction): string | null {
  switch (action) {
    ...
    case "user_action_required":
      return (
        "Ask the server administrator to resolve the host prerequisite, " +
...
function SystemConfirmationDialog({
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <strong id="system-confirmation-title">
            {systemConfirmationTitle(confirmation)}
          </strong>
          <span id="system-confirmation-detail">
            {systemConfirmationDetail(confirmation)}
          </span>
        </div>
        <div className="system-button-row">
...
function diagnosticScopeLabel(
  scope: DiagnosticReportV1["scopes"][number],
): string {
...
function RemoteWorkspaceDrawer({
  ...
  const [noProxy, setNoProxy] = useState(
    profile?.proxy.no_proxy.join(", ") ?? "",
  );
  const [dirty, setDirty] = useState(
    profile !== null && profile.authentication_kind !== "ssh_agent",
  );
    ...
    >
      ...
      >
        <div
          ...
          inert={guardedClose.confirming ? true : undefined}
          aria-hidden={guardedClose.confirming || undefined}
        >
          <div>
            <span className="panel-kicker">Remote workspace</span>
            <h2 id="workspace-settings-title">Server connection</h2>
          </div>
          <IconButton
            label="Close connection settings"
            onClick={guardedClose.requestClose}
          >
            <X size={18} />
          </IconButton>
        </div>
        ...
        >
            ...
            <div className="form-grid host-grid">
              ...
              <label>
                ...
                <input
                  ...
                  inputMode="numeric"
                  value={port}
                  onChange={update(setPort)}
                />
              </label>
            </div>
            <label>
              User name
              <input required value={user} onChange={update(setUser)} />
            </label>
            {requiredFieldMessage ? (
              <p
                className="form-error"
                id="workspace-required-fields"
                role="status"
              >
                {requiredFieldMessage}
              </p>
          ...
          <section className="form-section">
            ...
            {profile && profile.authentication_kind !== "ssh_agent" ? (
              <p className="form-error" role="alert">
                The saved authentication method is unavailable in this release.
                Save this workspace to use SSH agent.
              </p>
            ) : null}
          </section>
          <section className="form-section">
            ...
            <label>
              ...
              <input
                value={httpProxy}
                onChange={update(setHttpProxy)}
                placeholder="Optional HTTP origin"
              />
            </label>
            <label>
              HTTPS proxy
              <input
                value={httpsProxy}
                onChange={update(setHttpsProxy)}
                placeholder="Optional HTTPS origin"
              />
            </label>
            <label>
              Bypass proxy for
              <input
                value={noProxy}
                onChange={update(setNoProxy)}
                placeholder="localhost, example.org"
              />
            </label>
          </section>
        </div>
        {guardedClose.confirming ? (
          <DiscardChangesPrompt
            onKeep={guardedClose.keepEditing}
            onDiscard={guardedClose.discard}
          />
        ) : null}
        <div
        ...
        >
          ...
          <button
            ...
            type="button"
            aria-describedby={
              requiredFieldMessage ? "workspace-required-fields" : undefined
            }
            disabled={
              !valid ||
              busy ||
            ...
            title={
              !valid
                ? "Complete the required server fields"
                : streamEpoch === null
                  ? "Refresh this view before saving"
                  : profile && !dirty
                    ? "No unsaved changes"
            ...
            onClick={() => {
              ...
              const intent =
                pending?.canonicalPayload === canonicalProfilePayload(input)
              ...
              void onSave(intent).then((result) => {
                ...
                setPendingCreateObservation(
                  pendingRoute?.kind === "create" &&
                    pendingRoute.confirmedProfileId
                    ? {
                        profileId: pendingRoute.confirmedProfileId,
                        canonicalPayload:
                          result.pendingIntent!.canonicalPayload,
                      }
                    : null,
                );
              });
          ...
          >
            <Save size={15} /> {busy ? "Saving..." : "Save workspace"}
          </button>
        </div>
      </aside>
    </div>
...
function ProjectConfigurationDialog({
  ...
  const [hfModel, setHfModel] = useState(
    project?.execution.hf_model ?? DEFAULT_HF_MODEL,
  );
  const [codexModel, setCodexModel] = useState(
    project?.execution.codex_model ?? DEFAULT_CODEX_MODEL,
  );
  const [codexReasoningEffort, setCodexReasoningEffort] =
    useState<CodexReasoningEffortSelection>(() =>
      codexReasoningEffortSelection(project),
    );
  const [evolution, setEvolution] = useState<ProductEvolutionTargets>(
  ...
  const settlePendingSource = useCallback(
    async (outcome: "adopt" | "discard") => {
  ...
  const activeModeCapability = executionModeCapability(
    executionModeCapabilities,
    mode,
  );
  ...
  const modeCapabilities =
    capabilities && capabilityExecutionMode(capabilities) === mode
      ? capabilities
      : null;
  const incompleteSetup = project?.evolution_configuration_state === "pending";
  const capabilityMatchesDraft = Boolean(
    project &&
    capability &&
    capability.projectId === project.project_id &&
    capability.executionMode === mode,
  );
  const capabilityRetryable =
    capabilityMatchesDraft && capability?.status === "unavailable";
  const markDirty = () => {
  ...
  const selectSource = async (restoreFocus: boolean) => {
    ...
    try {
      ...
      if (activeSourceActionId.current === actionId)
        activeSourceActionId.current = null;
      ...
      if (sourceSelectionGeneration.current !== generation) return;
      if (!isWorkspaceSelectionCancelled(error))
        setSourceError(userMessage(error));
    } finally {
      if (activeSourceActionId.current === actionId)
        activeSourceActionId.current = null;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (activeActionId !== null)
        void onCancelSourceRef.current(activeActionId);
      const actionId = takePendingSourceAction();
      if (actionId !== null)
        void onSettleSourceRef.current(actionId, "discard");
    };
  }, [takePendingSourceAction]);
  const rows = evolutionTargetRows(modeCapabilities, evolution);
  const setupReady =
    !incompleteSetup ||
    (modeCapabilities !== null &&
      rows.every((row) => !row.selection.enabled || row.valid));
  const requiredFieldMessage =
    name.trim() === ""
      ? "Enter a project name."
      : creating && title.trim() === ""
        ? "Enter a task title."
        : creating && objective.trim() === ""
  ...
  const valid =
    name.trim().length > 0 &&
    (!creating || (title.trim().length > 0 && objective.trim().length > 0)) &&
  ...
  return (
    <div
      className={`drawer-backdrop ${creating ? "create-project-backdrop" : ""}`}
      role="presentation"
      onMouseDown={(event) => {
        if (guardedClose.confirming) {
          event.preventDefault();
    ...
    >
      <aside
        ref={dialogRef}
        className={`settings-drawer ${creating ? "create-project-dialog" : "edit-project-drawer"}`}
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        >
          <div>
            <span className="panel-kicker">
              {creating ? "New project" : "Current project settings"}
            </span>
            ...
            {creating ? (
              <p className="dialog-description">
                Choose the initial task, workspace, execution, and evolution
                settings for a separate project.
              </p>
            ) : (
              <p className="dialog-description">
                Changes apply only to the project currently open in the
                workspace.
              </p>
            )}
          </div>
          <IconButton
            label={creating ? "Cancel project creation" : "Close settings"}
        ...
        >
          <section className="form-section">
            <h3>{creating ? "Project details" : "Current project"}</h3>
            <label>
              Project name
              <input required value={name} onChange={change(setName)} />
            </label>
            {creating ? (
              <>
                <label>
                  Initial task title
                  <input required value={title} onChange={change(setTitle)} />
                </label>
                <label>
                ...
                <p className="form-help">
                  After creation, enter each later task from the Research
                  workspace.
                </p>
              </>
            ) : (
              <p className="form-help">
                Edit the next task from the Research workspace. This panel only
                changes the current project's workspace, execution, and
                evolution settings.
              </p>
            )}
            {requiredFieldMessage ? (
          ...
          <section className="form-section">
            ...
            >
              <button
                type="button"
                role="radio"
                aria-checked={source.kind === "scratch"}
                tabIndex={source.kind === "scratch" ? 0 : -1}
                className={source.kind === "scratch" ? "active" : ""}
                disabled={selectingSource || busy}
                onClick={() => {
                  invalidateSourceSelection();
                  void settlePendingSource("discard").then(() => {
                    setSource({
                      kind: "scratch",
                      display_name: "New workspace",
                      import_ref: null,
              ...
              <button
                ref={folderSourceButtonRef}
                type="button"
                role="radio"
                aria-checked={source.kind === "native_folder_snapshot"}
                tabIndex={source.kind === "native_folder_snapshot" ? 0 : -1}
                className={
                  source.kind === "native_folder_snapshot" ? "active" : ""
                }
                disabled={selectingSource || busy}
                onClick={(event) =>
                  void selectSource(
              ...
              >
                {selectingSource ? "Selecting..." : "Folder snapshot"}
              </button>
            </div>
            <div className="source-summary">
              ...
              <span>
                ...
                <small>
                  {source.kind === "scratch"
                    ? "A new managed workspace will be created."
                    : "A native snapshot reference is ready."}
                </small>
              </span>
            </div>
            {sourceError ? (
          ...
          <section className="form-section">
            ...
            <div
              ...
              onKeyDown={handleTablistKeyDown}
            >
              {visibleModeCapabilities.map((capability) => (
                <button
                  type="button"
                  role="radio"
                  key={capability.mode}
                  ...
                  aria-describedby={
                    capability.support_state === "supported"
                      ? undefined
                      : "execution-mode-support-message"
                  }
                  ...
                  <select
                    ...
                    onChange={(event) => {
                      setCodexReasoningEffort(
                        event.currentTarget
                          .value as CodexReasoningEffortSelection,
                      );
                      markDirty();
                    }}
                  >
            ...
            {activeModeCapability.support_state !== "supported" ? (
              <p
                className="mode-support-message"
                id="execution-mode-support-message"
                role="status"
              >
                {activeModeCapability.message}
              </p>
            ) : null}
            <p className="form-help">
              {mode === "codex_subscription_transcript"
                ? "Tasks run through the signed-in Codex subscription on the remote host."
                : "The remote host serves the selected model."}{" "}
              Sessions use transcript capture.
            </p>
          </section>
          <section className="form-section">
            <h3>Evolution targets</h3>
            {incompleteSetup && modeCapabilities ? (
              <div className="capability-ready" role="status">
                <CheckCircle2 size={15} />
                <p className="form-help">Remote evolution methods are ready.</p>
              </div>
            ) : null}
            {!modeCapabilities ? (
              <div className="capability-unavailable" role="status">
                <p className="form-help">
                  {project === null
                    ? "Remote methods will be loaded after the project session is prepared."
                    : capabilityMatchesDraft && capability?.status === "loading"
                      ? "Capabilities are loading for this project and mode."
                      : "Capabilities are unavailable for this project and mode."}
                </p>
                {capabilityRetryable ? (
                  <button
                    type="button"
                    ...
                    disabled={retryingCapabilities || busy}
                  >
                    <RefreshCw
                      className={retryingCapabilities ? "spin" : undefined}
                      size={14}
                    />{" "}
                    Retry capabilities
                  </button>
            ...
            <div className="target-list">
              {rows.map((row) => (
                <div
                  className={`target-toggle ${row.valid ? "" : "invalid"}`}
                ...
                >
                  <label>
                    <input
                      type="checkbox"
                      role="switch"
                      checked={row.selection.enabled}
                      disabled={!row.selection.enabled && !row.canEnable}
                      onChange={(event) => {
                        const enabled = event.currentTarget.checked;
                        setEvolution((current) => ({
                          ...current,
                          [row.targetId]: enabled
                            ? enableTarget(row)
                  ...
                  <select
                    ...
                    disabled={!row.capability || !row.selection.enabled}
                    onChange={(event) => {
                      ...
                      setEvolution((current) => ({
                        ...current,
                        [row.targetId]: {
                          enabled: row.selection.enabled,
                          method: selected.id,
                          config: selected.defaultConfig as OpenEvoJsonObject,
                        },
                      }));
                      markDirty();
                    }}
                  ...
                  {row.selection.enabled && row.selectedChoice?.configSchema ? (
                    <MethodConfigEditor
                      schema={row.selectedChoice.configSchema}
                      defaultConfig={
                        row.selectedChoice.defaultConfig as OpenEvoJsonObject
                      }
                      ...
                      onChange={(config) => {
                        setEvolution((current) => ({
                          ...current,
                          [row.targetId]: { ...row.selection, config },
                        }));
                        markDirty();
                      }}
                    />
                  ) : null}
                  {!row.valid && row.selection.enabled ? (
                    <small className="target-error">{row.reason}</small>
                  ) : null}
                  {!row.selection.enabled && !row.canEnable ? (
                    <small className="target-error">{row.reason}</small>
                  ) : null}
                </div>
              ))}
        ...
        >
          <button
            ...
            type="button"
            onClick={() => void reset()}
            disabled={!dirty || busy || selectingSource}
            title={!dirty ? "No unsaved changes" : "Undo changes"}
          >
          ...
          <button
            ...
            type="button"
            aria-describedby={
              requiredFieldMessage ? "project-required-fields" : undefined
            }
            disabled={
              !valid ||
              busy ||
              selectingSource ||
              (project !== null && !dirty && !incompleteSetup)
            }
            title={
              !profileId
                ? "Add a remote workspace first"
                : activeModeCapability.support_state !== "supported"
                  ? activeModeCapability.message
                  : !valid
                    ? "Complete all required fields and valid method settings"
                    : project && !dirty && !incompleteSetup
                      ? "No unsaved changes"
                      : creating
            ...
            onClick={() => {
              invalidateSourceSelection();
              const pendingActionId = pendingSourceActionId.current;
              void onSave(
                {
                  name: name.trim(),
                  ...(creating
                ...
                saveActionId.current,
                pendingActionId,
              ).then((result) => {
                if (result.replaceActionId)
                  saveActionId.current = newActionId();
                if (
                  pendingActionId !== null &&
                  result.pendingSourceOutcome !== null &&
                  pendingSourceActionId.current === pendingActionId
                ) {
                  pendingSourceActionId.current = null;
...
function InlineNotice({
  ...
  return (
    ...
    >
      {tone === "error" ? <XCircle size={18} /> : <AlertCircle size={18} />}
      <div>
        <strong>{title}</strong>
        <span>{detail}</span>
      </div>
...
function DiscardChangesPrompt({
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    const actions = Array.from(
      promptRef.current?.querySelectorAll<HTMLButtonElement>(
        "button:not([disabled])",
      ) ?? [],
    );
  ...
  return (
    <div className="discard-changes-modal" role="presentation">
      <div
        ref={promptRef}
        className="discard-changes-prompt"
        role="alertdialog"
        aria-modal="true"
      ...
      >
        <div>
          ...
          <span id="discard-detail">
            Your draft stays open until you choose to discard it.
          </span>
        </div>
        <button
          ref={keepRef}
...
function BlockingState({
  ...
  return (
    <div className="blocking-state" role="alert">
      <span className="product-mark large">
        <AlertCircle size={22} />
      </span>
      <h1>{title}</h1>
      <p>{detail}</p>
...
function evolutionTargetRows(
  ...
  const remoteIds =
    capabilities?.targets.map((target) => target.target_id) ?? [];
  const targetIds = [
    ...remoteIds,
    ...Object.keys(selections).filter(
      (targetId) => !remoteIds.includes(targetId),
    ),
  ];
  return targetIds.map((targetId) => {
    const capability =
      capabilities?.targets.find((target) => target.target_id === targetId) ??
      null;
    ...
    const visibleChoices: EvolutionChoiceRow[] =
    ...
    const resolverChoices: EvolutionChoiceRow[] =
      capability?.selection_resolvers.map((resolver) => {
        const supported =
          resolver.resolved_methods.length > 0 &&
          resolver.resolved_methods.every(
            (method) => method.support.overall === "supported",
          );
        return {
          id: resolver.selection_value,
          label: resolver.display_name,
          kind: "resolver",
          supported,
    ...
    if (!selected && selection.method !== null) {
      const accepted = capability?.accepted_methods.find(
        (method) => method.method_id === selection.method,
      );
      choices.unshift({
        id: selection.method,
        label: accepted
          ? `${selection.method} (existing selection)`
          : `${selection.method} (no longer available)`,
    ...
    const selectedChoice = choices.find(
      (choice) => choice.id === selection.method,
    );
    const defaultChoice = choices.find(
      (choice) =>
        choice.id === capability?.effective_default_method_id &&
        choice.kind === "method" &&
        choice.supported,
    );
    const configErrors = selectedChoice?.configSchema
      ? methodConfigErrors(
          selectedChoice.configSchema,
          selectedChoice.defaultConfig as OpenEvoJsonObject,
          selection.config as OpenEvoJsonObject,
        )
      : [];
    const valid =
      !selection.enabled ||
      Boolean(selectedChoice?.supported && configErrors.length === 0);
    const supportedSelection = Boolean(
      selectedChoice?.supported && selection.method !== null,
    );
    return {
      targetId,
      ...
      description:
        ...
        "This saved target is absent from current remote capabilities.",
      capability,
      selection,
      choices,
...
function enableTarget(
  row: EvolutionTargetConfigRow,
): ProductEvolutionTargets[string] {
  const selected = row.choices.find(
    (choice) => choice.id === row.selection.method,
  );
  if (selected?.supported && row.selection.method !== null)
    return { ...row.selection, enabled: true };
  const defaultChoice = row.choices.find(
    (choice) =>
      choice.id === row.capability?.effective_default_method_id &&
      choice.kind === "method" &&
      choice.supported,
  );
  ...
  return {
    enabled: true,
    method: defaultChoice.id,
    config: defaultChoice.defaultConfig as OpenEvoJsonObject,
  };
}
...
function projectSelectionIdentity(selection: ProjectSelection | null): string {
  ...
  if (selection.kind === "sample")
    return sampleOptionKey(selection.sampleProjectId);
...
function matchingConfirmedProfile(
  ...
  return (
    profiles?.find(
      (profile) =>
        profile.profile_id === confirmedProfileId &&
        canonicalProfile(profile) === canonicalPayload,
    ) ?? null
  );
}

function selectedArtifactsForRevision(
  artifacts: readonly ArtifactV1[],
  ...
  return artifacts
    .filter(
      (artifact) =>
        artifact.selected &&
        artifact.membership_revisions.some((member) =>
          sameRevisionRef(member, revision),
        ) &&
        !artifactRevisionRefs(artifact).some(
          (candidate) =>
            candidate.id === revision.id &&
            !sameRevisionRef(candidate, revision),
        ),
    )
    .sort((left, right) => {
...
function evolutionOutputRevisionForRun(
  ...
  const outputs = artifacts.filter(
    (artifact) =>
      ...
      artifact.membership_revisions.some((member) =>
        sameRevisionRef(member, artifact.produced_revision),
      ) &&
      !artifactRevisionRefs(artifact).some(
        (member) =>
          member.id === artifact.produced_revision.id &&
          !sameRevisionRef(member, artifact.produced_revision),
      ),
  );
  ...
  if (
    outputs.some(
      (artifact) => !sameRevisionRef(artifact.produced_revision, candidate),
    )
  )
    return null;
...
function authoritativeActiveRevision(
  ...
  if (!active || active.project_id !== project.remote?.core_project_id)
    return null;
  const matchingRunRefs = runs
    .flatMap(runRevisionRefs)
    .filter((revision) => revision.id === active.id);
  if (matchingRunRefs.some((revision) => !sameRevisionRef(revision, active)))
    return null;
  return active;
}
...
function runRevisionRefs(run: RunV1): RevisionRefV1[] {
  return [
    ...
    ...(run.revision_transition
      ? [
          run.revision_transition.predecessor_revision,
          run.revision_transition.successor_revision,
        ]
      : []),
  ];
}
...
function artifactContentIdentityError(
  artifact: ArtifactV1,
...
function artifactDiffIdentityError(
  artifact: ArtifactV1,
  diff: ArtifactDiffV1,
  artifacts: readonly ArtifactV1[],
): string | null {
  ...
  if (
    !artifact.lineage.source_artifact_ids.includes(diff.previous_artifact_id)
  ) {
    return "Artifact change history does not match the selected artifact. Refetch before viewing it.";
  }
  const previousMatches = artifacts.filter(
    (candidate) => candidate.id === diff.previous_artifact_id,
  );
...
function getProjectActivationReason(
  ...
  if (project.state === "archived")
    return "Archived projects cannot be activated.";
  const modeCapability = executionModeCapability(
    snapshot.executionModeCapabilities,
    project.execution.mode,
  );
  if (modeCapability.support_state !== "supported")
    return modeCapability.message;
  if (!profile || profile.profile_id !== project.profile_id)
    return "Configure this project's remote workspace before activation.";
  if (profile.connection_state !== "connected")
    return "Connect this project's remote workspace before activation.";
  const core = snapshot.state.core;
  if (core.profile_id !== project.profile_id)
    return "Connect this project's remote workspace before activation.";
  if (core.state === "online" || core.state === "degraded") return null;
  if (core.state === "offline" && core.failure?.code === "core_not_started")
    return null;
...
function getStartReason(
  ...
  if (project.evolution_configuration_state === "pending")
    return "Finish evolution setup before starting a session.";
  const modeCapability = executionModeCapability(
    snapshot.executionModeCapabilities,
    project.execution.mode,
  );
  if (modeCapability.support_state !== "supported")
    return modeCapability.message;
  if (snapshot.stream.status !== "fresh")
    return "Refresh this view before starting a session.";
  if (!profile || profile.profile_id !== project.profile_id)
    return "Configure this project's remote workspace before starting a session.";
  if (!project.remote || project.state !== "active")
    return "Activate this project on its assigned remote workspace before starting a session.";
  ...
  if (
    ...
    return "Activate this project on its assigned remote workspace before starting a session.";
  if (
    snapshot.state.core.state !== "online" ||
    !snapshot.state.core.active_tunnel ||
    snapshot.state.core.profile_id !== profile.profile_id ||
    active.connection_state !== "ready"
  )
    return "Connect this project's remote workspace before starting a session.";
  const capability = snapshot.capability;
  if (
    !capability ||
    capability.status !== "ready" ||
    capability.projectId !== project.project_id ||
    capability.executionMode !== project.execution.mode ||
    capability.value.project_id !== project.project_id ||
    capabilityExecutionMode(capability.value.capabilities) !==
      project.execution.mode
  )
    return "Remote capabilities are unavailable for this project and mode.";
  const invalidTarget = evolutionTargetRows(
    capability.value.capabilities,
    project.evolution.targets,
  ).find((row) => row.selection.enabled && !row.valid);
  if (invalidTarget) return invalidTarget.reason;
  const validation = snapshot.validation;
  if (
    !validation ||
    validation.status !== "ready" ||
    validation.projectId !== project.project_id ||
    validation.executionMode !== project.execution.mode ||
    validation.projectEtag !== project.etag ||
    validation.value.project_id !== project.project_id ||
    validation.value.project_etag !== project.etag ||
    validation.value.registry_digest !==
      capability.value.capabilities.registry_digest ||
    !validation.value.valid
  )
    return "Project validation is not current for this project and mode.";
  if (services.length === 0)
    return "Remote service status is unavailable. Open System and reconnect before starting a session.";
  if (services.some((service) => service.status !== "running"))
    return "Remote services need attention. Open System and reconnect before starting a session.";
...
function connectionLabel(
  ...
  const labels: Record<string, string> = {
    ...
    host_key_review: "Review required",
    checking: "Checking",
    bootstrapping: "Preparing",
...
function sessionTitle(run: RunV1, runs: readonly RunV1[]): string {
  const chronological = [...runs].sort((left, right) =>
    compareTimestampAndId(left.created_at, left.id, right.created_at, right.id),
  );
...
function sidebarSessionTitle(
  ...
  return (
    taskContentForRun(project, run, remembered)?.title ?? sessionTitle(run, runs)
  );
}
...
function canReadmitRun(
  ...
  const coreProjectId = snapshot?.projects.find(
  ...
  return (
    ...
    !snapshot.runs.some(
      (run) => run.project_id === coreProjectId && !isTerminal(run.status),
    )
  );
}
  ...
  const current =
    resource.kind === "profile"
      ...
      : refreshed.projects.find(
          (project) => project.project_id === resource.id,
        );
  return (
    refreshed.stream.epoch !== requestEpoch || current?.etag !== resource.etag
  );
}

function userMessage(error: unknown): string {
  if (error instanceof DesktopProductProviderUnavailableError)
    return "The local Desktop service is not ready. Restart OpenEvo Desktop and try again.";
  if (error instanceof DesktopApiError) return error.apiError.message;
  if (error instanceof DesktopProductAmbiguousMutationError)
...
function isAmbiguousRetryOutcome(error: unknown): boolean {
  return (
    error instanceof DesktopProductAmbiguousMutationError ||
    (!(error instanceof DesktopApiError) &&
...
function capabilityExecutionMode(
  capabilities: EvolutionCapabilitiesV1,
): ProjectV1["execution"]["mode"] {
  return capabilities.evaluated_profile.execution_mode === "self_deployed"
    ? "self-deployed"
...
function executionModeCapability(
  ...
  if (!capability)
    throw new Error("Desktop execution mode capability is missing.");
  return capability;
}
...
function firstSupportedExecutionMode(
  ...
  return (
    capabilities.modes.find((item) => item.support_state === "supported") ??
    null
  );
}
...
function parseCapabilityJsonObject(value: string): OpenEvoJsonObject {
  ...
  if (!isOpenEvoJsonObject(parsed))
    throw new Error("Remote capability JSON must be an object.");
  return parsed;
}
...
function isOpenEvoJsonValue(value: unknown): boolean {
  return (
    ...
    (Array.isArray(value) && value.every(isOpenEvoJsonValue)) ||
    isOpenEvoJsonObject(value)
  );
}
...
function handleTablistKeyDown(event: React.KeyboardEvent<HTMLElement>) {
  if (event.key === "Enter" || event.key === " ") {
    ...
    if (
      ...
      ["tab", "radio"].includes(active.getAttribute("role") ?? "") &&
  ...
  const nextIndex =
    event.key === "Home"
      ...
      : event.key === "End"
        ? choices.length - 1
        : (Math.max(0, currentIndex) + direction + choices.length) %
          choices.length;
  event.preventDefault();
