...
def test_incomplete_daemon_inputs_are_rebuilt_instead_of_reused(tmp_path: Path) -> None:
    repository = tmp_path / "repo"
    repository.mkdir()
    resource_script = repository / "scripts/ci/openevo_desktop_daemon_resource.py"
    resource_script.parent.mkdir(parents=True)
    resource_script.write_text("# test resource builder\n", encoding="utf-8")
    commit_root = tmp_path / "cache" / COMMIT
    inputs = commit_root / "daemon-inputs"
    inputs.mkdir(parents=True)
    (inputs / "interrupted-build").write_text("partial", encoding="utf-8")
    ...
    def fake_run(command: list[str], **_kwargs: object) -> object:
        ...
        if action == "build":
            for relative in (
                "core/openevo-0.1.10-py3-none-any.whl",
            ...
            (output / "daemon/openevo-daemon-bundle.json").write_text(
                json.dumps(
                    {
                        "core": {"registry_digest": "b" * 64},
                        "release": {"source_commit": COMMIT},
    ...
    formal_assets._build_release_assets(
    ...
    assert commands == ["build", "stage"]
    assert not (inputs / "interrupted-build").exists()
    assert len(list((inputs / "core").glob("openevo-*.whl"))) == 1
...
def test_prepare_uses_commit_scoped_formal_assets_without_legacy_daemon(
    ...
    assert prepared.development_snapshot_sha256 is None
...
def test_prepare_isolates_dirty_snapshot_from_clean_commit_cache(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    repository = tmp_path / "repo"
    repository.mkdir()
    ...
    prepared = formal_assets.prepare_formal_development_assets(
        repository_root=repository,
        source_commit=COMMIT,
        development_snapshot_sha256=snapshot,
        cache_root=tmp_path / "cache",
        managed_runtime_archive=runtime,
    )

    expected_root = tmp_path / "cache" / f"{COMMIT}-dirty-{snapshot}"
    assert prepared.development_snapshot_sha256 == snapshot
    assert prepared.release_assets_root == expected_root / "openevo-release-assets"
    assert prepared.askpass_helper == expected_root / "openevo-ssh-askpass"
